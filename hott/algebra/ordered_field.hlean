...
Definition mul_zero_lt_mul_inv_of_pos (H : 0 < a) : a * 0 < a * (1 / a).
  calc
  a * 0 = 0           : mul_zero
  ... < 1           : zero_lt_one
  ... = a * a^-1     : mul_inv_cancel (ne.symm (ne_of_lt H))
  ... = a * (1 / a) : inv_eq_one_div

Definition mul_zero_lt_mul_inv_of_neg (H : a < 0) : a * 0 < a * (1 / a).
  calc
  a * 0 = 0           : mul_zero
  ... < 1           : zero_lt_one
  ... = a * a^-1     : mul_inv_cancel (ne_of_lt H)
  ... = a * (1 / a) : inv_eq_one_div

Definition one_div_pos_of_pos (H : 0 < a) : 0 < 1 / a.
  lt_of_mul_lt_mul_left (mul_zero_lt_mul_inv_of_pos H) (le_of_lt H)

Definition one_div_neg_of_neg (H : a < 0) : 1 / a < 0.
  gt_of_mul_lt_mul_neg_left (mul_zero_lt_mul_inv_of_neg H) (le_of_lt H)


Definition le_mul_of_ge_one_right (Hb : b ≥ 0) (H : a ≥ 1) : b ≤ b * a.
  mul_one _ # (mul_le_mul_of_nonneg_left H Hb)

Definition lt_mul_of_gt_one_right (Hb : b > 0) (H : a > 1) : b < b * a.
  mul_one _ # (mul_lt_mul_of_pos_left H Hb)

Definition one_le_div_iff_le (a : A) {b : A} (Hb : b > 0) : 1 ≤ a / b ↔ b ≤ a.
...
Definition le_of_one_le_div (Hb : b > 0) (H : 1 ≤ a / b) : b ≤ a.
  (iff.mp (!one_le_div_iff_le Hb)) H

Definition one_le_div_of_le (Hb : b > 0) (H : b ≤ a) : 1 ≤ a / b.
  (iff.mpr (!one_le_div_iff_le Hb)) H

Definition one_lt_div_iff_lt (a : A) {b : A} (Hb : b > 0) : 1 < a / b ↔ b < a.
  have Hb' : b ≠ 0, from ne.symm (ne_of_lt Hb),
  iff.intro
  (assume H : 1 < a / b,
  calc
...
Definition lt_of_one_lt_div (Hb : b > 0) (H : 1 < a / b) : b < a.
  (iff.mp (!one_lt_div_iff_lt Hb)) H

Definition one_lt_div_of_lt (Hb : b > 0) (H : b < a) : 1 < a / b.
  (iff.mpr (!one_lt_div_iff_lt Hb)) H

Definition exists_lt (a : A) : Σ x, x < a.
  have H : a - 1 < a, from add_lt_of_le_of_neg (le.refl _) zero_gt_neg_one,
  sigma.mk _ H

Definition exists_gt (a : A) : Σ x, x > a.
  have H : a + 1 > a, from lt_add_of_le_of_pos (le.refl _) zero_lt_one,
  sigma.mk _ H

  (* the followingDefinitions amount to four iffs, for <, ≤, ≥, >. *)

Definition mul_le_of_le_div (Hc : 0 < c) (H : a ≤ b / c) : a * c ≤ b.
  !div_mul_cancel (ne.symm (ne_of_lt Hc)) # mul_le_mul_of_nonneg_right H (le_of_lt Hc)

Definition le_div_of_mul_le (Hc : 0 < c) (H : a * c ≤ b) : a ≤ b / c.
  calc
...
Definition le_mul_of_div_le (Hc : c > 0) (H : a / c ≤ b) : a ≤ b * c.
  calc
...
Definition mul_sub_mul_div_mul_neg (Hc : c ≠ 0) (Hd : d ≠ 0) (H : a / c < b / d) :
  (a * d - b * c) / (c * d) < 0.
...
Definition mul_sub_mul_div_mul_nonpos (Hc : c ≠ 0) (Hd : d ≠ 0) (H : a / c ≤ b / d) :
  (a * d - b * c) / (c * d) ≤ 0.
  have H1 : a / c - b / d ≤ 0, from calc
...
Definition div_lt_div_of_mul_sub_mul_div_neg (Hc : c ≠ 0) (Hd : d ≠ 0)
...
Proof. rewrite [zero_add at H3, sub_eq_add_neg at H3, neg_add_cancel_right at H3], exact H3 end

Definition div_le_div_of_mul_sub_mul_div_nonpos (Hc : c ≠ 0) (Hd : d ≠ 0)
  (H : (a * d - b * c) / (c * d) ≤ 0) : a / c ≤ b / d.
  have H1 : (a * d - c * b) / (c * d) ≤ 0,     by rewrite [mul.comm c b]; exact H,
  have H2 : a / c - b / d ≤ 0,                 by rewrite [!div_sub_div Hc Hd]; exact H1,
...
Definition div_pos_of_pos_of_pos (Ha : 0 < a) (Hb : 0 < b) : 0 < a / b.
Proof.
...
Definition div_nonneg_of_nonneg_of_pos (Ha : 0 ≤ a) (Hb : 0 < b) : 0 ≤ a / b.
Proof.
  rewrite div_eq_mul_one_div,
  apply mul_nonneg,
  exact Ha,
...
Definition div_neg_of_neg_of_pos (Ha : a < 0) (Hb : 0 < b) : a / b < 0.
Proof.
  rewrite div_eq_mul_one_div,
...
Definition div_nonpos_of_nonpos_of_pos (Ha : a ≤ 0) (Hb : 0 < b) : a / b ≤ 0.
Proof.
  rewrite div_eq_mul_one_div,
...
Definition div_neg_of_pos_of_neg (Ha : 0 < a) (Hb : b < 0) : a / b < 0.
Proof.
  rewrite div_eq_mul_one_div,
...
Definition div_nonpos_of_nonneg_of_neg (Ha : 0 ≤ a) (Hb : b < 0) :  a / b ≤ 0.
Proof.
  rewrite div_eq_mul_one_div,
...
Definition div_pos_of_neg_of_neg (Ha : a < 0) (Hb : b < 0) : 0 < a / b.
Proof.
  rewrite div_eq_mul_one_div,
...
Definition div_nonneg_of_nonpos_of_neg (Ha : a ≤ 0) (Hb : b < 0) : 0 ≤ a / b.
Proof.
  rewrite div_eq_mul_one_div,
...
Definition sub_self_div_two (a : A) : a - a / 2 = a / 2.
  by rewrite [-{a}add_halves at {1}, add_sub_cancel]

Definition add_midpoint {a b : A} (H : a < b) : a + (b - a) / 2 < b.
Proof.
...
Definition exists_add_lt_prod_pos_of_lt (H : b < a) : Σ c : A, b + c < a \* c > 0.
  sigma.mk ((a - b) / (1 + 1))
...
Definition ge_of_forall_ge_sub {a b : A} (H : forall ε : A, ε > 0 -> a ≥ b - ε) : a ≥ b.
Proof.
  apply le_of_not_gt,
  intro Hb,
...
Definition dec_eq_of_dec_lt : forall x y : A, decidable (x = y).
  take x y,
  decidable.by_cases
...
Definition le_of_one_div_le_one_div_of_neg (H : b < 0) (Hl : 1 / a ≤ 1 / b) : b ≤ a.
...
Definition lt_of_one_div_lt_one_div (H : 0 < a) (Hl : 1 / a < 1 / b) : b < a.
  have Hb : 0 < b, from pos_of_one_div_pos (calc
  0   < 1 / a : one_div_pos_of_pos H
  ... < 1 / b : Hl),
  have H : 1 < a / b, from (calc
...
Definition lt_of_one_div_lt_one_div_of_neg (H : b < 0) (Hl : 1 / a < 1 / b) : b < a.
...
Definition one_div_lt_one_div_of_lt (Ha : 0 < a) (H : a < b) : 1 / b < 1 / a.
  lt_of_not_ge
  (assume H',
  absurd H (not_lt_of_ge (le_of_one_div_le_one_div Ha H')))

Definition one_div_le_one_div_of_le (Ha : 0 < a) (H : a ≤ b) : 1 / b ≤ 1 / a.
  le_of_not_gt
  (assume H',
  absurd H (not_le_of_gt (lt_of_one_div_lt_one_div Ha H')))

Definition one_div_lt_one_div_of_lt_of_neg (Hb : b < 0) (H : a < b) : 1 / b < 1 / a.
  lt_of_not_ge
  (assume H',
  absurd H (not_lt_of_ge (le_of_one_div_le_one_div_of_neg Hb H')))

Definition one_div_le_one_div_of_le_of_neg (Hb : b < 0) (H : a ≤ b) : 1 / b ≤ 1 / a.
  le_of_not_gt
  (assume H',
  absurd H (not_le_of_gt (lt_of_one_div_lt_one_div_of_neg Hb H')))

Definition one_lt_one_div (H1 : 0 < a) (H2 : a < 1) : 1 < 1 / a.
  one_div_one # one_div_lt_one_div_of_lt H1 H2

Definition one_le_one_div (H1 : 0 < a) (H2 : a ≤ 1) : 1 ≤ 1 / a.
  one_div_one # one_div_le_one_div_of_le H1 H2

Definition one_div_lt_neg_one (H1 : a < 0) (H2 : -1 < a) : 1 / a < -1.
  one_div_neg_one_eq_neg_one # one_div_lt_one_div_of_lt_of_neg H1 H2

Definition one_div_le_neg_one (H1 : a < 0) (H2 : -1 ≤ a) : 1 / a ≤ -1.
  one_div_neg_one_eq_neg_one # one_div_le_one_div_of_le_of_neg H1 H2

Definition div_lt_div_of_pos_of_lt_of_pos (Hb : 0 < b) (H : b < a) (Hc : 0 < c) : c / a < c / b.
Proof.
...
Definition div_mul_le_div_mul_of_div_le_div_pos' {d e : A} (H : a / b ≤ c / d)
  (He : e > 0) : a / (b * e) ≤ c / (d * e).
Proof.
...
Definition abs_one_div (a : A) : abs (1 / a) = 1 / abs a.
  if H : a > 0 then
...
Definition sign_eq_div_abs (a : A) : sign a = a / (abs a).
  decidable.by_cases
  (suppose a = 0, by subst a; rewrite [zero_div, sign_zero])
  (suppose a ≠ 0,
  have abs a ≠ 0, from assume H, this (eq_zero_of_abs_eq_zero H),
  !eq_div_of_mul_eq this !eq_sign_mul_abs^-1)

Defined. discrete_linear_ordered_field
Defined. algebra
