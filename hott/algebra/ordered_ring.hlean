...
variable {A : Type}
namespace algebra
privateDefinition absurd_a_lt_a {B : Type} {a : A} [s : strict_order A] (H : a < a) : B.
...
(mul_le_mul_of_nonneg_left: forall a b c, le a b -> le zero c -> le (mul c a) (mul c b))
(mul_le_mul_of_nonneg_right: forall a b c, le a b -> le zero c -> le (mul a c) (mul b c))
(mul_lt_mul_of_pos_left: forall a b c, lt a b -> lt zero c -> lt (mul c a) (mul c b))
(mul_lt_mul_of_pos_right: forall a b c, lt a b -> lt zero c -> lt (mul a c) (mul b c))
...
Definition add_ab_group_of_ordered_semiring [trans_instance] (A : Type)
  [H : ordered_semiring A] : semiring A.
@ordered_semiring.to_semiring A H

Definition monoid_of_ordered_semiring [trans_instance] (A : Type)
...
Definition zero_lt_one : 0 < (1:A) . linear_ordered_semiring.zero_lt_one A
...
Definition le_iff_mul_le_mul_left (a b : A) {c : A} (H : c > 0) : a ≤ b ↔ c * a ≤ c * b.
  iff.intro
  (assume H', mul_le_mul_of_nonneg_left H' (le_of_lt H))
  (assume H', le_of_mul_le_mul_left H' H)

Definition le_iff_mul_le_mul_right (a b : A) {c : A} (H : c > 0) : a ≤ b ↔ a * c ≤ b * c.
  iff.intro
...
Definition nonneg_of_mul_nonneg_left (H : 0 ≤ a * b) (H1 : 0 < a) : 0 ≤ b.
  le_of_not_gt
  (assume H2 : b < 0,
  not_le_of_gt (mul_neg_of_pos_of_neg H1 H2) H)

Definition nonneg_of_mul_nonneg_right (H : 0 ≤ a * b) (H1 : 0 < b) : 0 ≤ a.
  le_of_not_gt
  (assume H2 : a < 0,
  not_le_of_gt (mul_neg_of_neg_of_pos H2 H1) H)

Definition neg_of_mul_neg_left (H : a * b < 0) (H1 : 0 ≤ a) : b < 0.
  lt_of_not_ge
  (assume H2 : b ≥ 0,
  not_lt_of_ge (mul_nonneg H1 H2) H)

Definition neg_of_mul_neg_right (H : a * b < 0) (H1 : 0 ≤ b) : a < 0.
  lt_of_not_ge
  (assume H2 : a ≥ 0,
  not_lt_of_ge (mul_nonneg H2 H1) H)

Definition nonpos_of_mul_nonpos_left (H : a * b ≤ 0) (H1 : 0 < a) : b ≤ 0.
  le_of_not_gt
  (assume H2 : b > 0,
  not_le_of_gt (mul_pos H1 H2) H)

Definition nonpos_of_mul_nonpos_right (H : a * b ≤ 0) (H1 : 0 < b) : a ≤ 0.
  le_of_not_gt
  (assume H2 : a > 0,
...
structure ordered_ring (A : Type) extends ring A, ordered_mul_ab_group A renaming
  ...
  zero_ne_one_class A.
(mul_nonneg : forall a b, le zero a -> le zero b -> le zero (mul a b))
(mul_pos : forall a b, lt zero a -> lt zero b -> lt zero (mul a b))

(* (* we make it a class now (and not as part of the structure) to avoid *)
(*   ordered_ring.to_ordered_mul_ab_group to be an instance *) *)


Definition add_ab_group_of_ordered_ring [trans_instance] (A : Type)
  [H : ordered_ring A] : ring A.
@ordered_ring.to_ring A H

Definition monoid_of_ordered_ring [trans_instance] (A : Type)
  [H : ordered_ring A] : ordered_ab_group A.
@ordered_ring.to_ordered_mul_ab_group A H

Definition zero_ne_one_class_of_ordered_ring [trans_instance] (A : Type)
...
Definition ordered_ring.to_ordered_semiring [trans_instance] [s : ordered_ring A] :
  ordered_semiring A.
( ordered_semiring, s,
  mul_zero                   . mul_zero,
  zero_mul                   . zero_mul,
...
Definition linear_ordered_comm_ring.eq_zero_sum_eq_zero_of_mul_eq_zero [s : linear_ordered_comm_ring A]
...
Definition zero_le_one : 0 ≤ (1:A) . one_mul 1 # mul_self_nonneg 1

Definition pos_prod_pos_sum_neg_prod_neg_of_mul_pos {a b : A} (Hab : a * b > 0) :
  (a > 0 \* b > 0) ⊎ (a < 0 \* b < 0).
  lt.by_cases
  (assume Ha : 0 < a,
  lt.by_cases
...
Definition le_of_mul_le_of_ge_one {a b c : A} (H : a * c ≤ b) (Hb : b ≥ 0) (Hc : c ≥ 1) : a ≤ b.
...
Definition nonneg_le_nonneg_of_squares_le {a b : A} (Ha : a ≥ 0) (Hb : b ≥ 0) (H : a * a ≤ b * b) :
  a ≤ b.
Proof.
  apply le_of_not_gt,
  intro Hab,
...
Defined.)
  (assume H1 : 0 > a,
  have H2 : -1 = 1, from (sign_of_neg H1)^-1 @ H,
  absurd ((eq_zero_of_neg_eq H2)^-1) zero_ne_one)

Definition eq_zero_of_sign_eq_zero (H : sign a = 0) : a = 0.
  lt.by_cases
  (assume H1 : 0 < a,
  absurd (H^-1 @ sign_of_pos H1) zero_ne_one)
  (assume H1 : 0 = a, H1^-1)
  (assume H1 : 0 > a,
  have H2 : 0 = -1, from H^-1 @ sign_of_neg H1,
  have H3 : 1 = 0, from eq_neg_of_eq_neg H2 @ neg_zero,
  absurd (H3^-1) zero_ne_one)

Definition neg_of_sign_eq_neg_one (H : sign a = -1) : a < 0.
  lt.by_cases
  (assume H1 : 0 < a,
  have H2 : -1 = 1, from H^-1 @ (sign_of_pos H1),
  absurd ((eq_zero_of_neg_eq H2)^-1) zero_ne_one)
  (assume H1 : 0 = a,
  have H2 : (0:A) = -1,
Proof.
...
Definition abs_dvd_of_dvd {a b : A} : a ∣ b -> abs a ∣ b.
  iff.mpr !abs_dvd_iff

Definition dvd_abs_iff (a b : A) : a ∣ abs b ↔ a ∣ b.
  abs.by_cases !iff.refl !dvd_neg_iff_dvd

Definition dvd_abs_of_dvd {a b : A} : a ∣ b -> a ∣ abs b.
  iff.mpr !dvd_abs_iff
...
Definition abs_mul_abs_self (a : A) : abs a * abs a = a * a.
  abs.by_cases rfl !neg_mul_neg

Definition abs_mul_self (a : A) : abs (a * a) = a * a.
  by rewrite [abs_mul, abs_mul_abs_self]

Definition sub_le_of_abs_sub_le_left (H : abs (a - b) ≤ c) : b - c ≤ a.
  if Hz : 0 ≤ a - b then
  (calc
...
Definition sub_le_of_abs_sub_le_right (H : abs (a - b) ≤ c) : a - c ≤ b.
  sub_le_of_abs_sub_le_left (!abs_sub # H)

Definition sub_lt_of_abs_sub_lt_left (H : abs (a - b) < c) : b - c < a.
  if Hz : 0 ≤ a - b then
  (calc
  a ≥ b : (iff.mp !sub_nonneg_iff_le) Hz
...
Definition sub_lt_of_abs_sub_lt_right (H : abs (a - b) < c) : a - c < b.
  sub_lt_of_abs_sub_lt_left (!abs_sub # H)

Definition abs_sub_square (a b : A) : abs (a - b) * abs (a - b) = a * a + b * b - (1 + 1) * a * b.
Proof.
...
Definition abs_abs_sub_abs_le_abs_sub (a b : A) : abs (abs a - abs b) ≤ abs (a - b).
Proof.
...
Definition pos_bit0_helper [s : linear_ordered_semiring A] (a : A) (H : a > 0) : bit0 a > 0.
  by rewrite ↑bit0; apply add_pos H H

Definition nonneg_bit0_helper [s : linear_ordered_semiring A] (a : A) (H : a ≥ 0) : bit0 a ≥ 0.
  by rewrite ↑bit0; apply add_nonneg H H

Definition pos_bit1_helper [s : linear_ordered_semiring A] (a : A) (H : a ≥ 0) : bit1 a > 0.
Proof.
  rewrite ↑bit1,
...
Definition nonneg_bit1_helper [s : linear_ordered_semiring A] (a : A) (H : a ≥ 0) : bit1 a ≥ 0.
  by apply le_of_lt; apply pos_bit1_helper _ H

Definition nonzero_of_pos_helper [s : linear_ordered_semiring A] (a : A) (H : a > 0) : a ≠ 0.
  ne_of_gt H

Definition nonzero_of_neg_helper [s : linear_ordered_ring A] (a : A) (H : a ≠ 0) : -a ≠ 0.
Proof. intro Ha, apply H, apply eq_of_neg_eq_neg, rewrite neg_zero, exact Ha end
