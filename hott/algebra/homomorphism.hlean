...
Definition is_add_hom_compose [has_add A] [has_add B] [has_add C]
  (f : B -> C) (g : A -> B) [is_add_hom f] [is_add_hom g] : is_add_hom (f o g).
...
Definition eq_zero_of_eq_zero_of_is_embedding {f : A -> B} [is_add_hom f] [is_embedding f]
  {a : A} (h : f a = 0) : a = 0.
  have f a = f 0, by rewrite [h, respect_zero],
  show a = 0, from is_injective_of_is_embedding this

Defined. add_group_A_B
...
Definition respect_inv (f : A -> B) [is_mul_hom f] (a : A) :
  f (a^-1) = (f a)^-1.
  have f (a^-1) * f a = 1, by rewrite [-respect_mul f, mul.left_inv, respect_one f],
...
Definition is_embedding_of_is_mul_hom (f : A -> B) [is_mul_hom f]
  ...
  (take x₁ x₂,
  suppose f x₁ = f x₂,
  have f (x₁ * x₂^-1) = 1, by rewrite [respect_mul f, respect_inv f, this, mul.right_inv],
  have x₁ * x₂^-1 = 1, from H _ this,
...
Proof. unfold is_ring_hom, apply _ end
...
Definition is_ring_hom_id : is_ring_hom (@id R₁).
  is_ring_hom.mk id (fun a₁ a₂ => rfl) (fun a₁ a₂ => rfl) rfl

Definition is_ring_hom_comp : is_ring_hom (g o f).
  is_ring_hom.mk _
