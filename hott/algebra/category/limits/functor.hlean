...
Definition functor_limit_object
    [H : has_limits_of_shape D I] (F : I ⇒ D ^c C) : C ⇒ D.
Proof.
  have lem : forall (c d : carrier C) (f : hom c d) (i j : carrier I) (k : i ⟶ j),
    (constant2_functor F d) k o to_fun_hom (F i) f o limit_morphism (constant2_functor F c) i =
      to_fun_hom (F j) f o limit_morphism (constant2_functor F c) j =>
Proof. intro c d f i j k, rewrite [-limit_commute _ k,▸*,+assoc,▸*,-naturality (F k) f] end,

  fapply functor.mk =>
   { intro c, exact limit_object (constant2_functor F c)} =>
   { intro c d f, fapply hom_limit,
     { intro i, refine to_fun_hom (F i) f o !limit_morphism} =>
     { apply lem}},
      ...
      { fapply cone_hom.mk,
        { fapply nat_trans.mk,
          ...
          { intro c d f, esimp, fapply @limit_cone_unique,
            { intro i, esimp, exact to_fun_hom (F i) f o η i c} =>
...
Definition is_complete_functor [instance] [H : is_complete D] : is_complete (D ^c C).
  ...
  (*Definition constant2_functor_op (F : I ⇒ (D ^c C)ᵒᵖ) (c : C) : I ⇒ D . *)
  (* proof *)
...
Definition functor_colimit_object
...
Definition functor_colimit_cone
      ...
      { fapply cone_hom.mk,
        { fapply nat_trans.mk,
          ...
          { intro c d f, esimp, fapply @colimit_cocone_unique,
            { intro i, esimp, exact η i d o to_fun_hom (F i) f} =>
