...
namespace yoneda
  ...
  (*
    ...
    However, we don't want to have them globally, because that will unfold the composition g o f
...
Definition contravariant_yoneda_embedding [reducible]
  ...
  *)
Definition yoneda_embedding (C : Precategory.{u v}) : C ⇒ cset ^c Cᵒᵖ.
...
Definition yoneda_lemma_hom_nat (c : C) (F : Cᵒᵖ ⇒ cset)
    (x : trunctype.carrier (F c)) {c₁ c₂ : Cᵒᵖ} (f : c₁ ⟶ c₂)
      : F f o yoneda_lemma_hom_fun c F x c₁ = yoneda_lemma_hom_fun c F x c₂ o to_fun_hom (ɏ c) f.
Proof.
    esimp [yoneda_embedding], apply eq_of_homotopy, intro f',
    refine _ @ ap (fun y => to_fun_hom F y x) !(@id_left _ C)^-1 =>
    exact ap10 !(@respect_comp Cᵒᵖ cset)^-1 x
Defined.

Definition yoneda_lemma_hom (c : C) (F : Cᵒᵖ ⇒ cset)
...
Definition yoneda_lemma_equiv (c : C)
...
Proof.
    ...
    { exact abstract begin intro η, esimp, apply nat_trans_eq,
      ...
      intro f,
      transitivity (F f o η c) id, reflexivity,
...
Definition yoneda_lemma_natural_ob (F : Cᵒᵖ ⇒ cset) {c c' : C} (f : c' ⟶ c)
    (η : ɏ c ⟹ F) :
     to_fun_hom (functor_lift of F) f (to_hom (yoneda_lemma c F) η) =
     to_hom (yoneda_lemma c' F) (η on to_fun_hom ɏ f).
Proof.
...
Defined.
  ...
  (* TODO: Investigate what is the bottleneck to type check the nextDefinition *)

  (* attribute yoneda_lemma functor_lift Precategory_Set precategory_Set homset *)
  (*   yoneda_embedding nat_trans.compose functor_nat_trans_compose *)
  (* attribute tlift functor.compose *)
Definition yoneda_lemma_natural_functor (c : C) (F F' : Cᵒᵖ ⇒ cset)
    (θ : F ⟹ F') (η : to_fun_ob ɏ c ⟹ F) :
     (functor_lift.{v u} ofn θ) c (to_hom (yoneda_lemma c F) η) =
     proof to_hom (yoneda_lemma c F') (θ on η) qed.
  by reflexivity

  (*Definition xx.{u v} {C : Precategory.{u v}} (c : C) (F F' : Cᵒᵖ ⇒ set) *)
  (*   (θ : F ⟹ F') (η : to_fun_ob ɏ c ⟹ F) : *)
  (*    proof _ qed = *)
  (*    to_hom (yoneda_lemma c F') (θ on η) . *)
  (* by reflexivity *)

  (*Definition yy.{u v} {C : Precategory.{u v}} (c : C) (F F' : Cᵒᵖ ⇒ set) *)
  (*   (θ : F ⟹ F') (η : to_fun_ob ɏ c ⟹ F) : *)
  (*    (functor_lift.{v u} ofn θ) c (to_hom (yoneda_lemma c F) η) = *)
  (*    proof _ qed . *)
  (* by reflexivity *)
...
Proof.
    ...
    { symmetry, transitivity _, apply @equiv_of_iso (homset _ _),
      exact @yoneda_lemma C c (ɏ c'), esimp [yoneda_embedding], exact !equiv_lift^-1ᵉ},
...
Proof.
    ...
    { exact !eq_equiv_iso @e !iso_equiv_F_iso_F @e !eq_equiv_iso^-1ᵉ},
    ...
    begin
      ...
      { unfold [is_representable],
        rexact fiber.sigma_char ɏ F @e sigma.sigma_equiv_sigma_right
                 (fun c => @eq_equiv_iso (cset ^c2 Cᵒᵖ) _ (hom_functor_left c) F)} =>
