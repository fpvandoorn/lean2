...
protectedDefinition sub_zero (n : ℕ) : n - 0 = n :=
rfl

Definition sub_succ (n m : ℕ) : n - succ m = pred (n - m) :=
rfl

protectedDefinition zero_sub (n : ℕ) : 0 - n = 0 :=
...
protectedDefinition sub_self (n : ℕ) : n - n = 0 :=
nat.rec_on n !nat.sub_zero (take k IH, !succ_sub_succ @ IH)

protectedDefinition add_sub_add_right (n k m : ℕ) : (n + k) - (m + k) = n - m :=
nat.rec_on k
  (calc
  ...
  (take l : nat,
    ...
    calc
      (n + succ l) - (m + succ l) = succ (n + l) - (m + succ l) : {!add_succ}
                              ...
                              ... =  n - m                      : IH)
protectedDefinition add_sub_add_left (k n m : ℕ) : (k + n) - (k + m) = n - m :=
add.comm n k # add.comm m k # nat.add_sub_add_right n k m

protectedDefinition add_sub_cancel (n m : ℕ) : n + m - m = n :=
nat.rec_on m
...
protectedDefinition add_sub_cancel_left (n m : ℕ) : n + m - n = m :=
!add.comm # !nat.add_sub_cancel

protectedDefinition sub_sub (n m k : ℕ) : n - m - k = n - (m + k) :=
nat.rec_on k
  (calc
...
protectedDefinition sub.right_comm (m n k : ℕ) : m - n - k = m - k - n :=
calc
...
protectedDefinition mul_sub_right_distrib (n m k : ℕ) : (n - m) * k = n * k - m * k :=
nat.rec_on m
  (calc
...
protectedDefinition mul_sub_left_distrib (n m k : ℕ) : n * (m - k) = n * m - n * k :=
calc
...
protectedDefinition mul_self_sub_mul_self_eq (a b : nat) : a * a - b * b = (a + b) * (a - b) :=
Proof.
...
protectedDefinition sub_add_cancel {n m : ℕ} : n ≥ m -> n - m + m = n :=
!add.comm # !add_sub_of_le

Definition sub_add_of_le {n m : ℕ} : n ≤ m -> n - m + m = m :=
!add.comm # add_sub_of_ge

Definition sub.cases {P : ℕ -> Type} {n m : ℕ} (H1 : n ≤ m -> P 0) (H2 : forall k, m + k = n -> P k)
  : P (n - m) :=
sum.elim !le.total
  (assume H3 : n ≤ m, (sub_eq_zero_of_le H3)^-1 # (H1 H3))
...
protectedDefinition add_sub_assoc {m k : ℕ} (H : k ≤ m) (n : ℕ) : n + m - k = n + (m - k) :=
...
sub.cases
  ...
  (take k : ℕ,
    assume H1 : m + k = n,
    assume H2 : k = 0,
    have H3 : n = m, from !add_zero # H2 # H1^-1,
    H3 # !le.refl)

Definition sub_sub.cases {P : ℕ -> ℕ -> Type} {n m : ℕ} (H1 : forall k, n = m + k -> P k 0)
  (H2 : forall k, m = n + k -> P 0 k) : P (n - m) (m - n) :=
sum.elim !le.total
  (assume H3 : n ≤ m,
    (sub_eq_zero_of_le H3)^-1 #  (H2 (m - n) (add_sub_of_le H3)^-1))
  (assume H3 : m ≤ n,
    (sub_eq_zero_of_le H3)^-1 # (H1 (n - m) (add_sub_of_le H3)^-1))

protectedDefinition sub_eq_of_add_eq {n m k : ℕ} (H : n + m = k) : k - n = m :=
...
protectedDefinition eq_sub_of_add_eq {a b c : ℕ} (H : a + c = b) : a = b - c :=
(nat.sub_eq_of_add_eq (!add.comm # H))^-1

protectedDefinition sub_eq_of_eq_add {a b c : ℕ} (H : a = c + b) : a - b = c :=
nat.sub_eq_of_add_eq (!add.comm # H^-1)

protectedDefinition sub_le_sub_right {n m : ℕ} (H : n ≤ m) (k : ℕ) : n - k ≤ m - k :=
obtain (l : ℕ) (Hl : n + l = m), from le.elim H,
sum.elim !le.total
  (assume H2 : n ≤ k, (sub_eq_zero_of_le H2)^-1 # !zero_le)
  (assume H2 : k ≤ n,
...
protectedDefinition sub_le_sub_left {n m : ℕ} (H : n ≤ m) (k : ℕ) : k - m ≤ k - n :=
obtain (l : ℕ) (Hl : n + l = m), from le.elim H,
sub.cases
...
protectedDefinition sub_pos_of_lt {m n : ℕ} (H : m < n) : n - m > 0 :=
have H1 : n = n - m + m, from (nat.sub_add_cancel (le_of_lt H))^-1,
have   H2 : 0 + m < n - m + m, begin rewrite [zero_add, -H1], exact H end,
!lt_of_add_lt_add_right H2

protectedDefinition lt_of_sub_pos {m n : ℕ} (H : n - m > 0) : m < n :=
lt_of_not_ge
  (take H1 : m ≥ n,
    have H2 : n - m = 0, from sub_eq_zero_of_le H1,
    !lt.irrefl (H2 # H))

protectedDefinition lt_of_sub_lt_sub_right {n m k : ℕ} (H : n - k < m - k) : n < m :=
lt_of_not_ge
  (assume H1 : m ≤ n,
    have H2 : m - k ≤ n - k, from nat.sub_le_sub_right H1 _,
    not_le_of_gt H H2)

protectedDefinition lt_of_sub_lt_sub_left {n m k : ℕ} (H : n - m < n - k) : k < m :=
lt_of_not_ge
  (assume H1 : m ≤ k,
    have H2 : n - k ≤ n - m, from nat.sub_le_sub_left H1 _,
    not_le_of_gt H H2)

protectedDefinition sub_lt_sub_add_sub (n m k : ℕ) : n - k ≤ (n - m) + (m - k) :=
sub.cases
  (assume H : n ≤ m, (zero_add (m - k))^-1 # nat.sub_le_sub_right H k)
  (take mn : ℕ,
    assume Hmn : m + mn = n,
    sub.cases
      (assume H : m ≤ k,
...
protectedDefinition sub_lt_self {m n : ℕ} (H1 : m > 0) (H2 : n > 0) : m - n < m :=
calc
...
protectedDefinition le_sub_of_add_le {m n k : ℕ} (H : m + k ≤ n) : m ≤ n - k :=
calc
  m = m + k - k : nat.add_sub_cancel
    ... ≤ n - k : nat.sub_le_sub_right H k

protectedDefinition lt_sub_of_add_lt {m n k : ℕ} (H : m + k < n) (H2 : k ≤ n) : m < n - k :=
lt_of_succ_le (nat.le_sub_of_add_le (calc
    succ m + k = succ (m + k) : succ_add_eq_succ_add
           ... ≤ n            : succ_le_of_lt H))

protectedDefinition sub_lt_of_lt_add {v n m : nat} (h₁ : v < n + m) (h₂ : n ≤ v) : v - n < m :=
...
Definition dist_eq_zero {n m : ℕ} (H : n = m) : dist n m = 0 :=
...
Proof. rewrite [add.comm k n, add.comm k m]; apply dist_add_add_right end
...
Proof. rewrite [add.comm (k - m) (m - n),
               {n - m + _ + _}add.assoc,
...
Proof. rewrite dist_eq_max_sub_min, apply sub_le end
