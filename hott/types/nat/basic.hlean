...
Definition add_eq_addl (x : ℕ) : forall y, x + y = x ⊕ y :=
nat.rec_on x
  (fun y => nat.rec_on y
    rfl
    (fun y₁ ih => calc
...
Definition pred_zero [simp] : pred 0 = 0 :=
rfl

Definition pred_succ [simp] (n : ℕ) : pred (succ n) = n :=
rfl

Definition eq_zero_sum_eq_succ_pred (n : ℕ) : n = 0 ⊎ n = succ (pred n) :=
nat.rec_on n
  (sum.inl rfl)
  (take m IH, sum.inr
    (show succ m = succ (pred (succ m)), from ap succ !pred_succ^-1))
...
Definition discriminate {B : Type} {n : ℕ} (H1: n = 0 -> B) (H2 : forall m, n = succ m -> B) : B :=
have H : n = n -> B, from nat.cases_on n H1 H2,
H rfl

Definition two_step_rec_on {P : ℕ -> Type} (a : ℕ) (H1 : P 0) (H2 : P 1)
    (H3 : forall (n : ℕ) (IH1 : P n) (IH2 : P (succ n)), P (succ (succ n))) : P a :=
...
Definition sub_induction {P : ℕ -> ℕ -> Type} (n m : ℕ) (H1 : forall m, P 0 m)
   (H2 : forall n, P (succ n) 0) (H3 : forall n m, P n m -> P (succ n) (succ m)) : P n m :=
have general : forall m, P n m, from nat.rec_on n H1
  (take k : ℕ,
    assume IH : forall m, P k m,
    take m : ℕ,
...
protectedDefinition add_zero [simp] (n : ℕ) : n + 0 = n :=
rfl

Definition add_succ [simp] (n m : ℕ) : n + succ m = succ (n + m) :=
rfl

protectedDefinition zero_add [simp] (n : ℕ) : 0 + n = n :=
Proof.
...
protectedDefinition add_comm [simp] (n m : ℕ) : n + m = m + n :=
Proof.
  induction n with n IH,
  { apply nat.zero_add},
...
protectedDefinition add_add (n l k : ℕ) : n + l + k = n + (k + l) :=
Proof.
...
protectedDefinition add_assoc [simp] (n m k : ℕ) : (n + m) + k = n + (m + k) :=
Proof.
...
protectedDefinition add_left_comm : forall (n m k : ℕ), n + (m + k) = m + (n + k) :=
left_comm nat.add_comm nat.add_assoc

protectedDefinition add_right_comm : forall (n m k : ℕ), n + m + k = n + k + m :=
right_comm nat.add_comm nat.add_assoc

protectedDefinition add_left_cancel {n m k : ℕ} : n + m = n + k -> m = k :=
nat.rec_on n
  (take H : 0 + m = 0 + k,
    !nat.zero_add^-1 @ H @ !nat.zero_add)
...
protectedDefinition add_right_cancel {n m k : ℕ} (H : n + m = k + m) : n = k :=
...
Definition eq_zero_prod_eq_zero_of_add_eq_zero {n m : ℕ} (H : n + m = 0) : n = 0 \* m = 0 :=
pair (eq_zero_of_add_eq_zero_right H) (eq_zero_of_add_eq_zero_left H)

Definition add_one [simp] (n : ℕ) : n + 1 = succ n := rfl
...
protectedDefinition mul_zero [simp] (n : ℕ) : n * 0 = 0 :=
rfl

Definition mul_succ [simp] (n m : ℕ) : n * succ m = n * m + n :=
rfl

(* commutativity, distributivity, associativity, identity *)

protectedDefinition zero_mul [simp] (n : ℕ) : 0 * n = 0 :=
nat.rec_on n
  !nat.mul_zero
  (take m IH, !mul_succ @ !nat.add_zero @ IH)

Definition succ_mul [simp] (n m : ℕ) : (succ n) * m = (n * m) + m :=
nat.rec_on m
...
protectedDefinition mul_comm [simp] (n m : ℕ) : n * m = m * n :=
nat.rec_on m
  (!nat.mul_zero @ !nat.zero_mul^-1)
  (take k IH, calc
    n * succ k = n * k + n    : mul_succ
           ... = k * n + n    : IH
           ... = (succ k) * n : succ_mul)

protectedDefinition right_distrib (n m k : ℕ) : (n + m) * k = n * k + m * k :=
nat.rec_on k
  (calc
...
protectedDefinition left_distrib (n m k : ℕ) : n * (m + k) = n * m + n * k :=
calc
...
protectedDefinition mul_assoc [simp] (n m k : ℕ) : (n * m) * k = n * (m * k) :=
nat.rec_on k
  (calc
...
protectedDefinition mul_one [simp] (n : ℕ) : n * 1 = n :=
calc
  n * 1 = n * 0 + n : mul_succ
    ... = 0 + n     : nat.mul_zero
    ... = n         : nat.zero_add

protectedDefinition one_mul [simp] (n : ℕ) : 1 * n = n :=
calc
  1 * n = n * 1 : nat.mul_comm
    ... = n     : nat.mul_one

Definition eq_zero_sum_eq_zero_of_mul_eq_zero {n m : ℕ} : n * m = 0 -> n = 0 ⊎ m = 0 :=
nat.cases_on n
        ...
        absurd
          (calc
            0 = succ n' * succ m' : H
             ...
             ... = succ (succ n' * m' + n') : add_succ)^-1
          !succ_ne_zero))

protectedDefinition comm_semiring [trans_instance] : comm_semiring nat :=
(comm_semiring,
 add            := nat.add,
