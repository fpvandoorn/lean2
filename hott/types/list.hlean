...
Definition append_nil_left (t : list T) : [] ++ t = t . idp

Definition append_cons (x : T) (s t : list T) : (x::s) ++ t = x :: (s ++ t) . idp

Definition append_nil_right : ∀ (t : list T), t ++ [] = t
| []       . rfl
| (a :: l) . calc
  (a :: l) ++ [] = a :: (l ++ []) : rfl
             ... = a :: l         : append_nil_right l

Definition append.assoc : ∀ (s t u : list T), s ++ t ++ u = s ++ (t ++ u)
| []       t u . rfl
| (a :: l) t u.
...
Definition length_nil : length (@nil T) = 0 . idp

Definition length_cons (x : T) (t : list T) : length (x::t) = length t + 1 . idp

Definition length_append : ∀ (s t : list T), length (s ++ t) = length s + length t
| []       t . calc
...
Definition eq_nil_of_length_eq_zero : ∀ {l : list T}, length l = 0 -> l = []
| []     H . rfl
| (a::s) H . by contradiction

Definition ne_nil_of_length_eq_succ : ∀ {l : list T} {n : nat}, length l = succ n -> l ≠ []
...
Definition concat : forall (x : T), list T -> list T
| a []       . [a]
| a (b :: l) . b :: concat a l

Definition concat_nil (x : T) : concat x [] = [x] . idp

Definition concat_cons (x y : T) (l : list T) : concat x (y::l)  = y::(concat x l) . idp

Definition concat_eq_append (a : T) : ∀ (l : list T), concat a l = l ++ [a]
| []       . rfl
| (b :: l).
  show b :: (concat a l) = (b :: l) ++ (a :: []),
  by rewrite concat_eq_append

Definition concat_ne_nil (a : T) : ∀ (l : list T), concat a l ≠ [].
by intro l; induction l; repeat contradiction

Definition length_concat (a : T) : ∀ (l : list T), length (concat a l) = length l + 1
| []      . rfl
| (x::xs) . by rewrite [concat_cons, *length_cons, length_concat]

Definition concat_append (a : T) : ∀ (l₁ l₂ : list T), concat a l₁ ++ l₂ = l₁ ++ a :: l₂
| []      . fun l₂ => rfl
| (x::xs) . fun l₂ => begin rewrite [concat_cons,append_cons, concat_append] end

Definition append_concat (a : T)  : ∀(l₁ l₂ : list T), l₁ ++ concat a l₂ = concat a (l₁ ++ l₂)
| []      . fun l₂ => rfl
| (x::xs) . fun l₂ => begin rewrite [+append_cons, concat_cons, append_concat] end

(* last *)

Definition last : forall l : list T, l ≠ [] -> T
| []          h . absurd rfl h
| [a]         h . a
...
Definition last_congr {l₁ l₂ : list T} (h₁ : l₁ ≠ []) (h₂ : l₂ ≠ []) (h₃ : l₁ = l₂)
  : last l₁ h₁ = last l₂ h₂.
apd011 last h₃ !is_prop.elimo

Definition last_concat {x : T} : ∀ {l : list T} (h : concat x l ≠ []), last (concat x l) h = x
| []          h . rfl
| [a]         h . rfl
| (a₁::a₂::l) h.
Proof.
...
Definition reverse_nil : reverse (@nil T) = [] . idp

Definition reverse_cons (x : T) (l : list T) : reverse (x::l) = concat x (reverse l) . idp

Definition reverse_singleton (x : T) : reverse [x] = [x] . idp

Definition reverse_append : ∀ (s t : list T), reverse (s ++ t) = (reverse t) ++ (reverse s)
| []         t2 . calc
...
Definition reverse_reverse : ∀ (l : list T), reverse (reverse l) = l
| []       . rfl
| (a :: l) . calc
...
Definition concat_eq_reverse_cons (x : T) (l : list T) : concat x l = reverse (x :: reverse l).
calc
  concat x l = concat x (reverse (reverse l)) : reverse_reverse
         ... = reverse (x :: reverse l)       : rfl

Definition length_reverse : ∀ (l : list T), length (reverse l) = length l
| []      . rfl
...
Definition head_cons [h : pointed T] (a : T) (l : list T) : head (a::l) = a . idp

Definition head_append [h : pointed T] (t : list T) : ∀ {s : list T}, s ≠ [] -> head (s ++ t) = head s
| []       H . absurd rfl H
| (a :: s) H.
...
Definition tail_nil : tail (@nil T) = [] . idp

Definition tail_cons (a : T) (l : list T) : tail (a::l) = l . idp

Definition cons_head_tail [h : pointed T] {l : list T} : l ≠ [] -> (head l)::(tail l) = l.
list.cases_on l
...
Definition mem_nil_iff (x : T) : x ∈ [] ↔ empty.
iff.intro down up

Definition not_mem_nil (x : T) : x ∉ [].
iff.mp !mem_nil_iff

Definition mem_cons (x : T) (l : list T) : x ∈ x :: l.
sum.inl rfl

Definition mem_cons_of_mem (y : T) {x : T} {l : list T} : x ∈ l -> x ∈ y :: l.
assume H, sum.inr H

Definition mem_cons_iff (x y : T) (l : list T) : x ∈ y::l ↔ (x = y ⊎ x ∈ l).
iff.rfl

Definition eq_or_mem_of_mem_cons {x y : T} {l : list T} : x ∈ y::l -> x = y ⊎ x ∈ l.
assume h, h

Definition mem_singleton {x a : T} : x ∈ [a] -> x = a.
suppose x ∈ [a], sum.rec_on (eq_or_mem_of_mem_cons this)
  (suppose x = a, this)
  (suppose x ∈ [], absurd this !not_mem_nil)

Definition mem_of_mem_cons_of_mem {a b : T} {l : list T} : a ∈ b::l -> b ∈ l -> a ∈ l.
assume ainbl binl, sum.rec_on (eq_or_mem_of_mem_cons ainbl)
  (suppose a = b, by substvars; exact binl)
  (suppose a ∈ l, this)

Definition mem_or_mem_of_mem_append {x : T} {s t : list T} : x ∈ s ++ t -> x ∈ s ⊎ x ∈ t.
...
Definition mem_append_of_mem_or_mem {x : T} {s t : list T} : (x ∈ s ⊎ x ∈ t) -> x ∈ s ++ t.
list.rec_on s
  (take H, sum.rec_on H (empty.elim o down) (assume H, H))
  (take y s,
...
Definition mem_append_iff (x : T) (s t : list T) : x ∈ s ++ t ↔ x ∈ s ⊎ x ∈ t.
iff.intro mem_or_mem_of_mem_append mem_append_of_mem_or_mem

Definition not_mem_of_not_mem_append_left {x : T} {s t : list T} : x ∉ s++t -> x ∉ s.
fun nxinst xins => absurd (mem_append_of_mem_or_mem (sum.inl xins)) nxinst

Definition not_mem_of_not_mem_append_right {x : T} {s t : list T} : x ∉ s++t -> x ∉ t.
fun nxinst xint => absurd (mem_append_of_mem_or_mem (sum.inr xint)) nxinst

Definition not_mem_append {x : T} {s t : list T} : x ∉ s -> x ∉ t -> x ∉ s++t.
fun nxins nxint xinst => sum.rec_on (mem_or_mem_of_mem_append xinst)
  (fun xins => by contradiction)
  (fun xint => by contradiction)
...
Definition mem_split {x : T} {l : list T} : x ∈ l -> Σs t : list T, l = s ++ (x::t).
list.rec_on l
...
Definition mem_append_left {a : T} {l₁ : list T} (l₂ : list T) : a ∈ l₁ -> a ∈ l₁ ++ l₂.
assume ainl₁, mem_append_of_mem_or_mem (sum.inl ainl₁)

Definition mem_append_right {a : T} (l₁ : list T) {l₂ : list T} : a ∈ l₂ -> a ∈ l₁ ++ l₂.
...
Definition mem_of_ne_of_mem {x y : T} {l : list T} (H₁ : x ≠ y) (H₂ : x ∈ y :: l) : x ∈ l.
sum.rec_on (eq_or_mem_of_mem_cons H₂) (fun e => absurd e H₁) (fun r => r)

Definition ne_of_not_mem_cons {a b : T} {l : list T} : a ∉ b::l -> a ≠ b.
assume nin aeqb, absurd (sum.inl aeqb) nin

Definition not_mem_of_not_mem_cons {a b : T} {l : list T} : a ∉ b::l -> a ∉ l.
...
Definition nil_sub (l : list T) : [] ⊆ l.
fun b i => empty.elim (iff.mp (mem_nil_iff b) i)

Definition sub.refl (l : list T) : l ⊆ l.
fun b i => i

Definition sub.trans {l₁ l₂ l₃ : list T} (H₁ : l₁ ⊆ l₂) (H₂ : l₂ ⊆ l₃) : l₁ ⊆ l₃.
fun b i => H₂ (H₁ i)

Definition sub_cons (a : T) (l : list T) : l ⊆ a::l.
fun b i => sum.inr i

Definition sub_of_cons_sub {a : T} {l₁ l₂ : list T} : a::l₁ ⊆ l₂ -> l₁ ⊆ l₂.
fun s b i => s b (mem_cons_of_mem _ i)

Definition cons_sub_cons  {l₁ l₂ : list T} (a : T) (s : l₁ ⊆ l₂) : (a::l₁) ⊆ (a::l₂).
fun b Hin => sum.rec_on (eq_or_mem_of_mem_cons Hin)
  (fun e : b = a =>  sum.inl e)
  (fun i : b ∈ l₁ => sum.inr (s i))

Definition sub_append_left (l₁ l₂ : list T) : l₁ ⊆ l₁++l₂.
fun b i => iff.mpr (mem_append_iff b l₁ l₂) (sum.inl i)

Definition sub_append_right (l₁ l₂ : list T) : l₂ ⊆ l₁++l₂.
fun b i => iff.mpr (mem_append_iff b l₁ l₂) (sum.inr i)

Definition sub_cons_of_sub (a : T) {l₁ l₂ : list T} : l₁ ⊆ l₂ -> l₁ ⊆ (a::l₂).
fun (s : l₁ ⊆ l₂) (x : T) (i : x ∈ l₁) => sum.inr (s i)

Definition sub_app_of_sub_left (l l₁ l₂ : list T) : l ⊆ l₁ -> l ⊆ l₁++l₂.
fun (s : l ⊆ l₁) (x : T) (xinl : x ∈ l) =>
  have x ∈ l₁, from s xinl,
  mem_append_of_mem_or_mem (sum.inl this)

Definition sub_app_of_sub_right (l l₁ l₂ : list T) : l ⊆ l₂ -> l ⊆ l₁++l₂.
fun (s : l ⊆ l₂) (x : T) (xinl : x ∈ l) =>
  have x ∈ l₂, from s xinl,
  mem_append_of_mem_or_mem (sum.inr this)

Definition cons_sub_of_sub_of_mem {a : T} {l m : list T} : a ∈ m -> l ⊆ m -> a::l ⊆ m.
fun (ainm : a ∈ m) (lsubm : l ⊆ m) (x : T) (xinal : x ∈ a::l) => sum.rec_on (eq_or_mem_of_mem_cons xinal)
  (suppose x = a, by substvars; exact ainm)
  (suppose x ∈ l, lsubm this)

Definition app_sub_of_sub_of_sub {l₁ l₂ l : list T} : l₁ ⊆ l -> l₂ ⊆ l -> l₁++l₂ ⊆ l.
fun (l₁subl : l₁ ⊆ l) (l₂subl : l₂ ⊆ l) (x : T) (xinl₁l₂ : x ∈ l₁++l₂) =>
...
Definition find_nil (x : T) : find x [] = 0 . idp

Definition find_cons (x y : T) (l : list T) : find x (y::l) = if x = y then 0 else succ (find x l).
idp

Definition find_cons_of_eq {x y : T} (l : list T) : x = y -> find x (y::l) = 0.
assume e, if_pos e

Definition find_cons_of_ne {x y : T} (l : list T) : x ≠ y -> find x (y::l) = succ (find x l).
assume n, if_neg n

(*Definition find_of_not_mem {l : list T} {x : T} : ¬x ∈ l -> find x l = length l.
list.rec_on l
   (suppose ¬x ∈ [], _)
   (take y l,
...
Definition nth_zero (a : T) (l : list T) : nth (a :: l) 0 = some a . idp

Definition nth_succ (a : T) (l : list T) (n : nat) : nth (a::l) (succ n) = nth l n . idp

Definition nth_eq_some : ∀ {l : list T} {n : nat}, n < length l -> Σ a : T, nth l n = some a
...
open decidable
Definition find_nth [h : decidable_eq T] {a : T} : ∀ {l}, a ∈ l -> nth l (find a l) = some a
| []     ain   . absurd ain !not_mem_nil
| (b::l) ainbl . by_cases
  (fun aeqb : a = b => by rewrite [find_cons_of_eq _ aeqb, nth_zero, aeqb])
  (fun aneb : a ≠ b => sum.rec_on (eq_or_mem_of_mem_cons ainbl)
    (fun aeqb : a = b => absurd aeqb aneb)
    (fun ainl : a ∈ l => by rewrite [find_cons_of_ne _ aneb, nth_succ, find_nth ainl]))
...
Definition inth_zero [h : pointed T] (a : T) (l : list T) : inth (a :: l) 0 = a . idp

Definition inth_succ [h : pointed T] (a : T) (l : list T) (n : nat) : inth (a::l) (n+1) = inth l n.
idp
Defined. nth

section ith
Definition ith : forall (l : list T) (i : nat), i < length l -> T
...
Definition qeq_app : ∀ (l₁ : list A) (a : A) (l₂ : list A), l₁++(a::l₂) ≈ a|l₁++l₂
| []      a l₂ . qhead a l₂
| (x::xs) a l₂ . qcons x (qeq_app xs a l₂)

Definition mem_head_of_qeq {a : A} {l₁ l₂ : list A} : l₁≈a|l₂ -> a ∈ l₁.
take q, qeq.rec_on q
  (fun l => !mem_cons)
  (fun b l l' q r => sum.inr r)

Definition mem_tail_of_qeq {a : A} {l₁ l₂ : list A} : l₁≈a|l₂ -> ∀ x, x ∈ l₂ -> x ∈ l₁.
take q, qeq.rec_on q
  (fun l x i => sum.inr i)
  (fun b l l' q r x xinbl => sum.rec_on (eq_or_mem_of_mem_cons xinbl)
     (fun xeqb : x = b => xeqb # mem_cons x l')
     (fun xinl : x ∈ l => sum.inr (r x xinl)))

(*
Definition mem_cons_of_qeq {a : A} {l₁ l₂ : list A} : l₁≈a|l₂ -> ∀ x, x ∈ l₁ -> x ∈ a::l₂.
take q, qeq.rec_on q
  (fun l x i => i)
  (fun b l l' q r x xinbl' => sum.elim_on (eq_or_mem_of_mem_cons xinbl')
    (fun xeqb  : x = b => xeqb # sum.inr (mem_cons x l))
    (fun xinl' : x ∈ l' => sum.rec_on (eq_or_mem_of_mem_cons (r x xinl'))
      (fun xeqa : x = a => xeqa # mem_cons x (b::l))
      (fun xinl : x ∈ l => sum.inr (sum.inr xinl))))*)

Definition length_eq_of_qeq {a : A} {l₁ l₂ : list A} : l₁≈a|l₂ -> length l₁ = succ (length l₂).
take q, qeq.rec_on q
  (fun l => rfl)
  (fun b l l' q r => by rewrite [*length_cons, r])

Definition qeq_of_mem {a : A} {l : list A} : a ∈ l -> (Σl', l≈a|l').
list.rec_on l
  (fun h : a ∈ nil => absurd h (not_mem_nil a))
  (fun x xs r ainxxs => sum.rec_on (eq_or_mem_of_mem_cons ainxxs)
    (fun aeqx  : a = x =>
...
Definition qeq_split {a : A} {l l' : list A} : l'≈a|l -> Σl₁ l₂, l = l₁++l₂ \* l' = l₁++(a::l₂).
take q, qeq.rec_on q
  (fun t =>
...
(*Definition sub_of_mem_of_sub_of_qeq {a : A} {l : list A} {u v : list A} : a ∉ l -> a::l ⊆ v -> v≈a|u -> l ⊆ u.
fun (nainl : a ∉ l) (s : a::l ⊆ v) (q : v≈a|u) (x : A) (xinl : x ∈ l) =>
...
Definition map_nil (f : A -> B) : map f [] = [] . idp

Definition map_cons (f : A -> B) (a : A) (l : list A) : map f (a :: l) = f a :: map f l . idp

lemma map_concat (f : A -> B) (a : A) : forall l, map f (concat a l) = concat (f a) (map f l)
| nil    . rfl
| (b::l) . begin rewrite [concat_cons, +map_cons, concat_cons, map_concat] end

lemma map_append (f : A -> B) : forall l₁ l₂, map f (l₁++l₂) = (map f l₁)++(map f l₂)
| nil    . take l, rfl
| (a::l) . take l', begin rewrite [append_cons, *map_cons, append_cons, map_append] end

lemma map_reverse (f : A -> B) : forall l, map f (reverse l) = reverse (map f l)
| nil    . rfl
...
Definition map_id : forall l : list A, map id l = l
| []      . rfl
| (x::xs) . begin rewrite [map_cons, map_id] end

Definition map_id' {f : A -> A} (H : forall x, f x = x) : forall l : list A, map f l = l
| []      . rfl
| (x::xs) . begin rewrite [map_cons, H, map_id'] end

Definition map_map (g : B -> C) (f : A -> B) : forall l, map g (map f l) = map (g o f) l
| []       . rfl
| (a :: l).
  show (g o f) a :: map g (map f l) = map (g o f) (a :: l),
  by rewrite (map_map l)

Definition length_map (f : A -> B) : forall l : list A, length (map f l) = length l
| []       . by esimp
| (a :: l).
  show length (map f l) + 1 = length l + 1,
  by rewrite (length_map l)

Definition mem_map {A B : Type} (f : A -> B) : forall {a l}, a ∈ l -> f a ∈ map f l
...
Definition exists_of_mem_map {A B : Type} {f : A -> B} {b : B} :
    forall {l}, b ∈ map f l -> Σa, a ∈ l \* f a = b
| []     H . empty.elim (down H)
...
Definition eq_of_map_const {A B : Type} {b₁ b₂ : B} : forall {l : list A}, b₁ ∈ map (const A b₂) l -> b₁ = b₂
...
Definition filter_nil (p : A -> Type) [h : decidable_pred p] : filter p [] = [] . idp

Definition filter_cons_of_pos {p : A -> Type} [h : decidable_pred p] {a : A} : forall l, p a -> filter p (a::l) = a :: filter p l.
fun l pa => if_pos pa

Definition filter_cons_of_neg {p : A -> Type} [h : decidable_pred p] {a : A} : forall l, ¬ p a -> filter p (a::l) = filter p l.
fun l pa => if_neg pa

(*
Definition of_mem_filter {p : A -> Type} [h : decidable_pred p] {a : A} : forall {l}, a ∈ filter p l -> p a
| []     ain . absurd ain !not_mem_nil
| (b::l) ain . by_cases
  (assume pb  : p b,
...
Definition mem_of_mem_filter {p : A -> Type} [h : decidable_pred p] {a : A} : forall {l}, a ∈ filter p l -> a ∈ l
| []     ain . absurd ain !not_mem_nil
| (b::l) ain . by_cases
  (fun pb  : p b =>
    have a ∈ b :: filter p l, by rewrite [filter_cons_of_pos _ pb at ain]; exact ain,
...
Definition mem_filter_of_mem {p : A -> Type} [h : decidable_pred p] {a : A} : forall {l}, a ∈ l -> p a -> a ∈ filter p l
| []     ain pa . absurd ain !not_mem_nil
| (b::l) ain pa . by_cases
  (fun pb  : p b => sum.rec_on (eq_or_mem_of_mem_cons ain)
    (fun aeqb : a = b => by rewrite [filter_cons_of_pos _ pb, aeqb]; exact !mem_cons)
    (fun ainl : a ∈ l => by rewrite [filter_cons_of_pos _ pb]; exact (mem_cons_of_mem _ (mem_filter_of_mem ainl pa))))
  (fun npb : ¬ p b => sum.rec_on (eq_or_mem_of_mem_cons ain)
    (fun aeqb : a = b => absurd (eq.rec_on aeqb pa) npb)
    (fun ainl : a ∈ l => by rewrite [filter_cons_of_neg _ npb]; exact (mem_filter_of_mem ainl pa)))

Definition filter_sub {p : A -> Type} [h : decidable_pred p] (l : list A) : filter p l ⊆ l.
fun a ain => mem_of_mem_filter ain

Definition filter_append {p : A -> Type} [h : decidable_pred p] : forall (l₁ l₂ : list A), filter p (l₁++l₂) = filter p l₁ ++ filter p l₂
| []      l₂ . rfl
| (a::l₁) l₂ . by_cases
...
Definition foldl_nil (f : A -> B -> A) (a : A) : foldl f a [] = a . idp

Definition foldl_cons (f : A -> B -> A) (a : A) (b : B) (l : list B) : foldl f a (b::l) = foldl f (f a b) l . idp

Definition foldr (f : A -> B -> B) : B -> list A -> B
| b []       . b
| b (a :: l) . f a (foldr b l)

Definition foldr_nil (f : A -> B -> B) (b : B) : foldr f b [] = b . idp

Definition foldr_cons (f : A -> B -> B) (b : B) (a : A) (l : list A) : foldr f b (a::l) = f a (foldr f b l) . idp

section foldl_eq_foldr
  (* foldl and foldr coincide when f is commutative and associative *)
  parameters {α : Type} {f : α -> α -> α}
  hypothesis (Hcomm  : forall a b, f a b = f b a)
  hypothesis (Hassoc : forall a b c, f (f a b) c = f a (f b c))
  include Hcomm Hassoc

Definition foldl_eq_of_comm_of_assoc : forall a b l, foldl f a (b::l) = f b (foldl f a l)
  | a b  nil    . Hcomm a b
  | a b  (c::l).
    begin
...
Definition foldl_eq_foldr : forall a l, foldl f a l = foldr f a l
  | a nil      . rfl
  | a (b :: l).
    begin
      rewrite foldl_eq_of_comm_of_assoc,
...
Defined. foldl_eq_foldr

Definition foldl_append (f : B -> A -> B) : forall (b : B) (l₁ l₂ : list A), foldl f b (l₁++l₂) = foldl f (foldl f b l₁) l₂
| b []      l₂ . rfl
| b (a::l₁) l₂ . by rewrite [append_cons, *foldl_cons, foldl_append]

Definition foldr_append (f : A -> B -> B) : forall (b : B) (l₁ l₂ : list A), foldr f b (l₁++l₂) = foldr f (foldr f b l₂) l₁
| b []      l₂ . rfl
