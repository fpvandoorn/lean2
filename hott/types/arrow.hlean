...
Definition arrow_functor [unfold_full] : (A -> B) -> (A' -> B') . pi_functor f0 (fun a => f1)

  (* Equivalences *)

Definition is_equiv_arrow_functor
    [H0 : is_equiv f0] [H1 : is_equiv f1] : is_equiv (arrow_functor f0 f1).
  is_equiv_pi_functor f0 (fun a => f1)

Definition arrow_equiv_arrow_rev (f0 : A' <~> A) (f1 : B <~> B')
    : (A -> B) <~> (A' -> B').
...
Definition arrow_equiv_of_is_contr_left [H : is_contr A] : (A -> B) <~> B.
Proof.
    fapply equiv.MK,
...
Definition arrow_equiv_of_is_contr_right [H : is_contr B] : (A -> B) <~> unit.
...
Definition arrow_empty_left : (empty -> B) <~> unit.
  !pi_empty_left

Definition arrow_unit_left : (unit -> B) <~> B.
  !arrow_equiv_of_is_contr_left

Definition arrow_unit_right : (A -> unit) <~> unit.
...
Definition arrow_transport {B C : A -> Type} (p : a = a') (f : B a -> C a)
    : (transport (fun a => B a -> C a) p f) == (fun b => p # f (p^-1 # b)).
  eq.rec_on p (fun x => idp)
...
Definition arrow_pathover_left {B C : A -> Type} {f : B a -> C a} {g : B a' -> C a'} {p : a = a'}
    (r : forall (b : B a), f b =[p] g (p # b)) : f =[p] g.
Proof.
...
Definition arrow_pathover_right {B C : A -> Type} {f : B a -> C a} {g : B a' -> C a'} {p : a = a'}
    (r : forall (b' : B a'), f (p^-1 # b') =[p] g b') : f =[p] g.
Proof.
    cases p, apply pathover_idp_of_eq,
    apply eq_of_homotopy, intro b,
    exact eq_of_pathover_idp (r b),
...
Definition arrow_pathover_constant_left {B : Type} {C : A -> Type} {f : B -> C a} {g : B -> C a'}
    {p : a = a'} (r : forall (b : B), f b =[p] g b) : f =[p] g.
  pi_pathover_constant r

Definition arrow_pathover_constant_right' {B : A -> Type} {C : Type}
    {f : B a -> C} {g : B a' -> C} {p : a = a'}
    (r : forall (b : B a) (b' : B a') (q : b =[p] b'), f b = g b') : f =[p] g.
  arrow_pathover (fun b b' q => pathover_of_eq p (r q))

Definition arrow_pathover_constant_right {B : A -> Type} {C : Type} {f : B a -> C}
    {g : B a' -> C} {p : a = a'} (r : forall (b : B a), f b = g (p # b)) : f =[p] g.
  arrow_pathover_left (fun b => pathover_of_eq p (r b))

Definition arrow_pathover_constant_right_rev {A : Type} {B : A -> Type} {C : Type} {a a' : A}
    {f : B a -> C} {g : B a' -> C} {p : a = a'} (r : forall (b : B a'), f (p^-1 # b) = g b) : f =[p] g.
  arrow_pathover_right (fun b => pathover_of_eq p (r b))
...
Definition apo11_arrow_pathover_constant_right {f : D a -> A'} {g : D a' -> A'} {p : a = a'}
    {q : d =[p] d'} (r : forall (d : D a), f d = g (p # d))
