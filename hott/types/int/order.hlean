...
privateDefinition nonneg (a : ℤ) : Type₀ := int.cases_on a (take n, unit) (take n, empty)
protectedDefinition le (a b : ℤ) : Type₀ := nonneg (b - a)

Definition int_has_le [instance] [priority int.prio]: has_le int :=
has_le.mk int.le

protectedDefinition lt (a b : ℤ) : Type₀ := (a + 1) ≤ b

Definition int_has_lt [instance] [priority int.prio]: has_lt int :=
has_lt.mk int.lt

local attribute nonneg [reducible]
privateDefinition decidable_nonneg [instance] (a : ℤ) : decidable (nonneg a) := int.cases_on a _ _
...
privateDefinition nonneg.elim {a : ℤ} : nonneg a -> Σn : ℕ, a = n :=
int.cases_on a (take n H, sigma.mk n rfl) (take n', empty.elim)

privateDefinition nonneg_sum_nonneg_neg (a : ℤ) : nonneg a ⊎ nonneg (-a) :=
int.cases_on a (take n, sum.inl star) (take n, sum.inr star)

Definition le.intro {a b : ℤ} {n : ℕ} (H : a + n = b) : a ≤ b :=
have n = b - a, from eq_add_neg_of_add_eq (begin rewrite [add.comm, H] end), (* !add.comm # H), *)
show nonneg (b - a), from this # star

Definition le.elim {a b : ℤ} (H : a ≤ b) : Σn : ℕ, a + n = b :=
obtain (n : ℕ) (H1 : b - a = n), from nonneg.elim H,
sigma.mk n (!add.comm # iff.mpr !add_eq_iff_eq_add_neg (H1^-1))

protectedDefinition le_total (a b : ℤ) : a ≤ b ⊎ b ≤ a :=
sum.imp_right
  (assume H : nonneg (-(b - a)),
...
Definition of_nat_le_of_nat_of_le {m n : ℕ} (H : #nat m ≤ n) : of_nat m ≤ of_nat n :=
obtain (k : ℕ) (Hk : m + k = n), from nat.le.elim H,
le.intro (Hk # (of_nat_add m k)^-1)

Definition le_of_of_nat_le_of_nat {m n : ℕ} (H : of_nat m ≤ of_nat n) : (#nat m ≤ n) :=
obtain (k : ℕ) (Hk : of_nat m + of_nat k = of_nat n), from le.elim H,
have m + k = n, from of_nat.inj (of_nat_add m k @ Hk),
nat.le.intro this

Definition of_nat_le_of_nat_iff (m n : ℕ) : of_nat m ≤ of_nat n ↔ m ≤ n :=
iff.intro le_of_of_nat_le_of_nat of_nat_le_of_nat_of_le

Definition lt_add_succ (a : ℤ) (n : ℕ) : a < a + succ n :=
...
Definition lt.intro {a b : ℤ} {n : ℕ} (H : a + succ n = b) : a < b :=
H # lt_add_succ a n

Definition lt.elim {a b : ℤ} (H : a < b) : Σn : ℕ, a + succ n = b :=
...
Definition of_nat_lt_of_nat_iff (n m : ℕ) : of_nat n < of_nat m ↔ n < m :=
calc
...
Definition lt_of_of_nat_lt_of_nat {m n : ℕ} (H : of_nat m < of_nat n) : #nat m < n :=
iff.mp !of_nat_lt_of_nat_iff H

Definition of_nat_lt_of_nat_of_lt {m n : ℕ} (H : #nat m < n) : of_nat m < of_nat n :=
iff.mpr !of_nat_lt_of_nat_iff H

(* show that the integers form an ordered additive group *)

protectedDefinition le_refl (a : ℤ) : a ≤ a :=
le.intro (add_zero a)

protectedDefinition le_trans {a b c : ℤ} (H1 : a ≤ b) (H2 : b ≤ c) : a ≤ c :=
...
have a + of_nat (n + m) = c, from
  calc
    a + of_nat (n + m) = a + (of_nat n + m) : {of_nat_add n m}
      ... = a + n + m : (add.assoc a n m)^-1
      ... = b + m : {Hn}
      ... = c : Hm,
le.intro this

protectedDefinition le_antisymm : forall {a b : ℤ}, a ≤ b -> b ≤ a -> a = b :=
...
protectedDefinition lt_irrefl (a : ℤ) : ¬ a < a :=
(suppose a < a,
...
protectedDefinition ne_of_lt {a b : ℤ} (H : a < b) : a ≠ b :=
(suppose a = b, absurd (this # H) (int.lt_irrefl b))

Definition le_of_lt {a b : ℤ} (H : a < b) : a ≤ b :=
obtain (n : ℕ) (Hn : a + succ n = b), from lt.elim H,
le.intro Hn

protectedDefinition lt_iff_le_prod_ne (a b : ℤ) : a < b ↔ (a ≤ b \* a ≠ b) :=
iff.intro
...
protectedDefinition le_iff_lt_sum_eq (a b : ℤ) : a ≤ b ↔ (a < b ⊎ a = b) :=
iff.intro
  (assume H,
    by_cases
...
Definition lt_succ (a : ℤ) : a < a + 1 :=
int.le_refl (a + 1)

protectedDefinition add_le_add_left {a b : ℤ} (H : a ≤ b) (c : ℤ) : c + a ≤ c + b :=
obtain (n : ℕ) (Hn : a + n = b), from le.elim H,
...
protectedDefinition add_lt_add_left {a b : ℤ} (H : a < b) (c : ℤ) : c + a < c + b :=
let H' := le_of_lt H in
(iff.mpr (int.lt_iff_le_prod_ne _ _)) (pair (int.add_le_add_left H' _)
                                  (take Heq, let Heq' := add_left_cancel Heq in
                                   !int.lt_irrefl (Heq' # H)))

protectedDefinition mul_nonneg {a b : ℤ} (Ha : 0 ≤ a) (Hb : 0 ≤ b) : 0 ≤ a * b :=
...
protectedDefinition mul_pos {a b : ℤ} (Ha : 0 < a) (Hb : 0 < b) : 0 < a * b :=
...
protectedDefinition zero_lt_one : (0 : ℤ) < 1 := star

protectedDefinition not_le_of_gt {a b : ℤ} (H : a < b) : ¬ b ≤ a :=
  assume Hba,
  let Heq := int.le_antisymm (le_of_lt H) Hba in
  !int.lt_irrefl (Heq # H)

protectedDefinition lt_of_lt_of_le {a b c : ℤ} (Hab : a < b) (Hbc : b ≤ c) : a < c :=
...
protectedDefinition lt_of_le_of_lt  {a b c : ℤ} (Hab : a ≤ b) (Hbc : b < c) : a < c :=
  let Hbc' := le_of_lt Hbc in
  let Hac := int.le_trans Hab Hbc' in
  (iff.mpr !int.lt_iff_le_prod_ne) (pair Hac
    (assume Heq, int.not_le_of_gt (Heq^-1 # Hbc) Hab))

protectedDefinition linear_ordered_comm_ring [trans_instance] :
...
protectedDefinition decidable_linear_ordered_comm_ring [instance] :
...
Definition of_nat_nonneg (n : ℕ) : 0 ≤ of_nat n := star

Definition of_nat_pos {n : ℕ} (Hpos : #nat n > 0) : of_nat n > 0 :=
of_nat_lt_of_nat_of_lt Hpos

Definition of_nat_succ_pos (n : nat) : of_nat (nat.succ n) > 0 :=
of_nat_pos !nat.succ_pos

Definition exists_eq_of_nat {a : ℤ} (H : 0 ≤ a) : Σn : ℕ, a = of_nat n :=
obtain (n : ℕ) (H1 : 0 + of_nat n = a), from le.elim H,
sigma.mk n (!zero_add # (H1^-1))

Definition exists_eq_neg_of_nat {a : ℤ} (H : a ≤ 0) : Σn : ℕ, a = -(of_nat n) :=
have -a ≥ 0, from iff.mpr !neg_nonneg_iff_nonpos H,
obtain (n : ℕ) (Hn : -a = of_nat n), from exists_eq_of_nat this,
sigma.mk n (eq_neg_of_eq_neg (Hn^-1))

Definition of_nat_nat_abs_of_nonneg {a : ℤ} (H : a ≥ 0) : of_nat (nat_abs a) = a :=
obtain (n : ℕ) (Hn : a = of_nat n), from exists_eq_of_nat H,
Hn^-1 # ap of_nat (nat_abs_of_nat n)

Definition of_nat_nat_abs_of_nonpos {a : ℤ} (H : a ≤ 0) : of_nat (nat_abs a) = -a :=
have -a ≥ 0, from iff.mpr !neg_nonneg_iff_nonpos H,
calc
  of_nat (nat_abs a) = of_nat (nat_abs (-a)) : nat_abs_neg
                 ... = -a                    : of_nat_nat_abs_of_nonneg this

Definition of_nat_nat_abs (b : ℤ) : nat_abs b = abs b :=
sum.elim (le.total 0 b)
  (assume H : b ≥ 0, of_nat_nat_abs_of_nonneg H @ (abs_of_nonneg H)^-1)
  (assume H : b ≤ 0, of_nat_nat_abs_of_nonpos H @ (abs_of_nonpos H)^-1)

Definition nat_abs_abs (a : ℤ) : nat_abs (abs a) = nat_abs a :=
abs.by_cases rfl !nat_abs_neg

Definition lt_of_add_one_le {a b : ℤ} (H : a + 1 ≤ b) : a < b :=
obtain (n : nat) (H1 : a + 1 + n = b), from le.elim H,
have a + succ n = b, by rewrite [-H1, add.assoc, add.comm 1],
lt.intro this

Definition add_one_le_of_lt {a b : ℤ} (H : a < b) : a + 1 ≤ b :=
obtain (n : nat) (H1 : a + succ n = b), from lt.elim H,
have a + 1 + n = b, by rewrite [-H1, add.assoc, add.comm 1],
le.intro this

Definition lt_add_one_of_le {a b : ℤ} (H : a ≤ b) : a < b + 1 :=
lt_add_of_le_of_pos H star

Definition le_of_lt_add_one {a b : ℤ} (H : a < b + 1) : a ≤ b :=
have H1 : a + 1 ≤ b + 1, from add_one_le_of_lt H,
le_of_add_le_add_right H1

Definition sub_one_le_of_lt {a b : ℤ} (H : a ≤ b) : a - 1 < b :=
lt_of_add_one_le (begin rewrite sub_add_cancel, exact H end)

Definition lt_of_sub_one_le {a b : ℤ} (H : a - 1 < b) : a ≤ b :=
!sub_add_cancel # add_one_le_of_lt H

Definition le_sub_one_of_lt {a b : ℤ} (H : a < b) : a ≤ b - 1 :=
le_of_lt_add_one begin rewrite sub_add_cancel, exact H end

Definition lt_of_le_sub_one {a b : ℤ} (H : a ≤ b - 1) : a < b :=
!sub_add_cancel # (lt_add_one_of_le H)

Definition sign_of_succ (n : nat) : sign (nat.succ n) = 1 :=
sign_of_pos (of_nat_pos !nat.succ_pos)

Definition exists_eq_neg_succ_of_nat {a : ℤ} : a < 0 -> Σm : ℕ, a = -[1+m] :=
int.cases_on a
  (take (m : nat) H, absurd (of_nat_nonneg m : 0 ≤ m) (not_le_of_gt H))
  (take (m : nat) H, sigma.mk m rfl)

Definition eq_one_of_mul_eq_one_right {a b : ℤ} (H : a ≥ 0) (H' : a * b = 1) : a = 1 :=
...
Definition eq_one_of_mul_eq_one_left {a b : ℤ} (H : b ≥ 0) (H' : a * b = 1) : b = 1 :=
eq_one_of_mul_eq_one_right H (!mul.comm # H')

Definition eq_one_of_mul_eq_self_left {a b : ℤ} (Hpos : a ≠ 0) (H : b * a = a) : b = 1 :=
eq_of_mul_eq_mul_right Hpos (H @ (one_mul a)^-1)

Definition eq_one_of_mul_eq_self_right {a b : ℤ} (Hpos : b ≠ 0) (H : b * a = b) : a = 1 :=
eq_one_of_mul_eq_self_left Hpos (!mul.comm # H)

Definition eq_one_of_dvd_one {a : ℤ} (H : a ≥ 0) (H' : a ∣ 1) : a = 1 :=
dvd.elim H'
  (take b,
    suppose 1 = a * b,
    eq_one_of_mul_eq_one_right H this^-1)

Definition exists_least_of_bdd {P : ℤ -> Type} [HP : decidable_pred P]
    (Hbdd : Σ b : ℤ, forall z : ℤ, z ≤ b -> ¬ P z)
        (Hinh : Σ z : ℤ, P z) : Σ lb : ℤ, P lb \* (forall z : ℤ, z < lb -> ¬ P z) :=
Proof.
    cases Hbdd with [b, Hb],
    cases Hinh with [elt, Helt],
    existsi b + of_nat (least (fun n => P (b + of_nat n)) (nat.succ (nat_abs (elt - b)))),
    ...
    have Hk : nat_abs (z - b) < least (fun n => P (b + of_nat n)) (nat.succ (nat_abs (elt - b))), begin
    ...
    rewrite Hzbk,
    apply fun p => mt (ge_least_of_lt _ p) Hk',
    apply nat.lt_trans Hk,
    apply least_lt _ !lt_succ_self H'
Defined.

Definition exists_greatest_of_bdd {P : ℤ -> Type} [HP : decidable_pred P]
    (Hbdd : Σ b : ℤ, forall z : ℤ, z ≥ b -> ¬ P z)
        (Hinh : Σ z : ℤ, P z) : Σ ub : ℤ, P ub \* (forall z : ℤ, z > ub -> ¬ P z) :=
Proof.
    cases Hbdd with [b, Hb],
    cases Hinh with [elt, Helt],
    existsi b - of_nat (least (fun n => P (b - of_nat n)) (nat.succ (nat_abs (b - elt)))),
    have Heltb : elt < b, begin
      apply lt_of_not_ge,
      intro Hge,
      apply (Hb _ Hge) Helt
    ...
    have Hk : nat_abs (b - z) < least (fun n => P (b - of_nat n)) (nat.succ (nat_abs (b - elt))), begin
