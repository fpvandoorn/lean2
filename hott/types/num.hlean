...
namespace pos_num
Definition succ_not_is_one (a : pos_num) : is_one (succ a) = ff.
  pos_num.rec_on a rfl (take n iH, rfl) (take n iH, rfl)

Definition succ_one                : succ one = bit0 one
Definition succ_bit1 (a : pos_num) : succ (bit1 a) = bit0 (succ a)
Definition succ_bit0 (a : pos_num) : succ (bit0 a) = bit1 a

Definition ne_of_bit0_ne_bit0 {a b : pos_num} (H₁ : bit0 a ≠ bit0 b) : a ≠ b.
  suppose a = b,
  absurd rfl (this # H₁)

Definition ne_of_bit1_ne_bit1 {a b : pos_num} (H₁ : bit1 a ≠ bit1 b) : a ≠ b.
  suppose a = b,
  absurd rfl (this # H₁)

Definition pred_succ : forall (a : pos_num), pred (succ a) = a
  | one      . rfl
...
Definition one_add_one     : one + one = bit0 one
Definition one_add_bit0    : one + (bit0 a) = bit1 a
Definition one_add_bit1    : one + (bit1 a) = succ (bit1 a)
Definition bit0_add_one    : (bit0 a) + one = bit1 a
Definition bit1_add_one    : (bit1 a) + one = succ (bit1 a)
Definition bit0_add_bit0   : (bit0 a) + (bit0 b) = bit0 (a + b)
Definition bit0_add_bit1   : (bit0 a) + (bit1 b) = bit1 (a + b)
Definition bit1_add_bit0   : (bit1 a) + (bit0 b) = bit1 (a + b)
Definition bit1_add_bit1   : (bit1 a) + (bit1 b) = succ (bit1 (a + b))
Definition one_mul         : one * a = a
Defined.

Definition mul_one         : forall a, a * one = a
  | one      . rfl
  | (bit1 n).
...
Definition decidable_eq [instance] : forall (a b : pos_num), decidable (a = b)
  | one      one      . inl rfl
...
Definition lt_one_right_eq_ff : forall a : pos_num, a ≮ one
  | one      . rfl
  | (bit0 a) . rfl
  | (bit1 a) . rfl

Definition lt_one_succ_eq_tt : forall a : pos_num, one < succ a
  | one      . rfl
  | (bit0 a) . rfl
  | (bit1 a) . rfl

Definition lt_of_lt_bit0_bit0 {a b : pos_num} (H : bit0 a < bit0 b) : a < b . H
Definition lt_of_lt_bit0_bit1 {a b : pos_num} (H : bit1 a < bit0 b) : a < b . H
Definition lt_of_lt_bit1_bit1 {a b : pos_num} (H : bit1 a < bit1 b) : a < b . H
Definition lt_of_lt_bit1_bit0 {a b : pos_num} (H : bit0 a < bit1 b) : a < succ b . H

Definition lt_bit0_bit0_eq_lt (a b : pos_num) : lt (bit0 a) (bit0 b) = lt a b.
  rfl

Definition lt_bit1_bit1_eq_lt (a b : pos_num) : lt (bit1 a) (bit1 b) = lt a b.
  rfl

Definition lt_bit1_bit0_eq_lt (a b : pos_num) : lt (bit1 a) (bit0 b) = lt a b.
  rfl

Definition lt_bit0_bit1_eq_lt_succ (a b : pos_num) : lt (bit0 a) (bit1 b) = lt a (succ b).
  rfl

Definition lt_irrefl : forall (a : pos_num), a ≮ a
  | one      . rfl
  | (bit0 a).
    begin
...
Definition ne_of_lt_eq_tt : forall {a b : pos_num}, a < b -> a = b -> empty
...
Definition lt_base : forall a : pos_num, a < succ a
  | one      . rfl
  | (bit0 a).
    begin
...
Definition lt_step : forall {a b : pos_num}, a < b -> a < succ b
  | one      one      H . rfl
  | one      (bit0 b) H . rfl
  | one      (bit1 b) H . rfl
...
Definition lt_of_lt_succ_succ : forall {a b : pos_num}, succ a < succ b -> a < b
...
Definition lt_succ_succ : forall {a b : pos_num}, a < b -> succ a < succ b
  | one      one      H . absurd H ff_ne_tt
  | one      (bit0 b) H.
    begin
...
Definition lt_of_lt_succ : forall {a b : pos_num}, succ a < b -> a < b
...
Definition lt_of_lt_succ_of_ne : forall {a b : pos_num}, a < succ b -> a ≠ b -> a < b
...
Definition lt_trans : forall {a b c : pos_num}, a < b -> b < c -> a < c
...
Definition lt_antisymm : forall {a b : pos_num}, a < b -> b ≮ a
  | one      one      H . rfl
  | one      (bit0 b) H . rfl
  | one      (bit1 b) H . rfl
  | (bit0 a) one      H . absurd H ff_ne_tt
...
Definition le_refl : forall a : pos_num, a ≤ a.
  lt_base

Definition le_eq_lt_succ {a b : pos_num} : le a b = lt a (succ b).
  rfl

Definition not_lt_of_le : forall {a b : pos_num}, a ≤ b -> b < a -> empty
...
Definition le_antisymm : forall {a b : pos_num}, a ≤ b -> b ≤ a -> a = b
  | one      one      H₁ H₂ . rfl
  | one      (bit0 b) H₁ H₂.
...
Definition le_trans {a b c : pos_num} : a ≤ b -> b ≤ c -> a ≤ c.
Proof.
    intro H₁ H₂,
...
Definition decidable_eq [instance] : forall (a b : num), decidable (a = b)
  | zero zero       . inl rfl
