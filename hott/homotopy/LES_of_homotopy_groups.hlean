...
PART 1:
We define a sequence fiber_sequence as inDefinition 8.4.3.
It has types X(n) : pType
X(0)   . Y,
X(1)   . X,
X(n+1) . fiber (f(n))
...
PART 2:
Now we can define a new fiber sequence X'(n) : pType, and here we slightly diverge from the book.
We define it as
X'(0)   . Y,
X'(1)   . X,
X'(2)   . fiber f
...
e(k)   . 1               for k = 0,1,2,3
e(k+3) . Ω(e(k)) o (-)^-1 for k > 0

Now the sequence (X', f' o e) is equivalent to (X, f), Hence (X', f' o e) is an exact sequence.
...
slightly shifted:
eᵣ = (-)^-1 for n ≡ 3, 4, 5 mod 6                       and eᵣ = 1 otherwise
e  = (-)^-1 for n ≡ 4, 5, 6 mod 6 (except for n = 0)    and e  = 1 otherwise
eₗ = (-)^-1 for n ≡ 5, 6, 7 mod 6 (except for n = 0, 1) and eₗ = 1 otherwise
...
this equality is notDefinitionally true. Hence we cannot even state whether f'(3n) = Ωⁿ(f) without
...
Definition fiber_sequence_helper (v : Σ(X Y : pType), X ->* Y)
    : Σ(Z X : pType), Z ->* X.
  ⟨pfiber v.2.2, v.1, ppoint v.2.2⟩

Definition fiber_sequence_helpern (v : Σ(X Y : pType), X ->* Y) (n : ℕ)
    : Σ(Z X : pType), Z ->* X.
...
Definition fiber_sequence_carrier (n : ℕ) : pType.
...
Definition is_exact_fiber_sequence : is_exact_t fiber_sequence.
  fun n x p => fiber.mk (fiber.mk x p) rfl
...
Definition fiber_sequence_carrier_pequiv_eq (n : ℕ)
    (x : fiber_sequence_carrier (n+1)) (p : fiber_sequence_fun n x = (point _))
    (q : fiber_sequence_fun (n+1) (fiber.mk x p) = (point _))
    : fiber_sequence_carrier_pequiv n (fiber.mk (fiber.mk x p) q)
      = !point_eq^-1 @ ap (fiber_sequence_fun n) q^-1 @ p.
  fiber_ppoint_equiv_eq p q

Definition fiber_sequence_carrier_pequiv_inv_eq (n : ℕ)
    (p : Ω(fiber_sequence_carrier n)) : (fiber_sequence_carrier_pequiv n)^-1ᵉ* p =
      fiber.mk (fiber.mk (point _) (point_eq (fiber_sequence_fun n) @ p)) idp.
...
Definition fiber_sequence_fun_eq_helper (n : ℕ)
    ...
    fiber_sequence_carrier_pequiv n
      (fiber_sequence_fun (n + 3)
        ((fiber_sequence_carrier_pequiv (n + 1))^-1ᵉ* p)) =
          Ω-> (fiber_sequence_fun n) p^-1.
Proof.
    refine ap (fun x => fiber_sequence_carrier_pequiv n (fiber_sequence_fun (n + 3) x))
    ...
    refine fiber_sequence_carrier_pequiv_eq n (point _) (point_eq (fiber_sequence_fun n)) _ @ _ =>
    esimp,
    apply whisker_right,
    apply whisker_left,
...
Definition fiber_sequence_carrier_pequiv_eq_point_eq_idp (n : ℕ) :
    fiber_sequence_carrier_pequiv_eq n
      (Point (fiber_sequence_carrier (n+1)))
      (point_eq (fiber_sequence_fun n))
      (point_eq (fiber_sequence_fun (n + 1))) = idp.
Proof.
    apply con_inv_eq_idp,
    refine ap (fun x => whisker_left _ (_ @ x)) _ @ _,
    { reflexivity},
    { reflexivity},
    refine ap (whisker_left _)
              (eq_transport_Fl_idp_left (fiber_sequence_fun n)
                                        (point_eq (fiber_sequence_fun n))) @ _ =>
    apply whisker_left_idp_con_eq_assoc
Defined.

Definition fiber_sequence_fun_phomotopy_helper (n : ℕ) :
    (fiber_sequence_carrier_pequiv n o*
      fiber_sequence_fun (n + 3)) o*
        (fiber_sequence_carrier_pequiv (n + 1))^-1ᵉ* ==*
          Ω-> (fiber_sequence_fun n) o* pinverse.
Proof.
    fapply Build_pHomotopy,
    { exact chain_complex.fiber_sequence_fun_eq_helper f n} =>
    { esimp, rewrite [idp_con], refine _ @ whisker_left _ (concat_1p _)^-1,
      apply whisker_right,
      apply whisker_left,
      exact chain_complex.fiber_sequence_carrier_pequiv_eq_point_eq_idp f n}
Defined.

Definition fiber_sequence_fun_eq (n : ℕ) : forall ,
    fiber_sequence_carrier_pequiv n (fiber_sequence_fun (n + 3) x) =
      Ω-> (fiber_sequence_fun n) (fiber_sequence_carrier_pequiv (n + 1) x)^-1.
Proof.
...
Definition fiber_sequence_fun_phomotopy (n : ℕ) :
    fiber_sequence_carrier_pequiv n o*
      fiber_sequence_fun (n + 3) ==*
          (Ω-> (fiber_sequence_fun n) o* pinverse) o* fiber_sequence_carrier_pequiv (n + 1).
Proof.
    apply phomotopy_of_pinv_right_phomotopy,
    apply fiber_sequence_fun_phomotopy_helper
Defined.

Definition boundary_map : loops Y ->* pfiber f.
  fiber_sequence_fun 2 o* (fiber_sequence_carrier_pequiv 0)^-1ᵉ*
...
Definition loop_spaces : ℕ -> pType
  | 0     . Y
  | 1     . X
  | 2     . pfiber f
  | (k+3) . loops (loop_spaces k)

  (* The maps between the homotopy groups *)
Definition loop_spaces_fun : forall , loop_spaces (n+1) ->* loop_spaces n
  | 0     . proof f qed
...
Definition fiber_sequence_pequiv_loop_spaces :
    forall n, fiber_sequence_carrier n <~>* loop_spaces n
  | 0     . by reflexivity
  | 1     . by reflexivity
  | 2     . by reflexivity
  | (k+3).
...
Definition fiber_sequence_pequiv_loop_spaces_add3 (n : ℕ)
    : fiber_sequence_pequiv_loop_spaces (n + 3) =
      Ω-> (fiber_sequence_pequiv_loop_spaces n) o* fiber_sequence_carrier_pequiv n.
  by reflexivity
...
Definition pid_or_pinverse : forall (n : ℕ), loop_spaces n <~>* loop_spaces n
  | 0     . pequiv.rfl
  | 1     . pequiv.rfl
  | 2     . pequiv.rfl
  | 3     . pequiv.rfl
...
Definition pid_or_pinverse_add4_rev (n : ℕ) :
    pid_or_pinverse (n + 4) ==* pinverse o* Ω->(pid_or_pinverse (n + 1)).
  !ap1_pcompose_pinverse

Definition fiber_sequence_phomotopy_loop_spaces : forall (n : ℕ),
    fiber_sequence_pequiv_loop_spaces n o* fiber_sequence_fun n ==*
      (loop_spaces_fun n o* pid_or_pinverse (n + 1)) o* fiber_sequence_pequiv_loop_spaces (n + 1)
  | 0     . proof proof (reflexivity _) qed @* pwhisker_right _ !pcompose_pid^-1* qed
  | 1     . by reflexivity
  | 2    .
    begin
      refine !pid_pcompose @* _,
      replace loop_spaces_fun 2 with boundary_map =>
      refine _ @* pwhisker_left _ fiber_sequence_pequiv_loop_spaces_3_phomotopy^-1*,
      apply phomotopy_of_pinv_right_phomotopy,
      exact !pcompose_pid^-1*
    end
  | (k+3).
    begin
      ...
      refine _ @* !passoc^-1*,
      refine _ @* pwhisker_left _ !passoc^-1*,
      refine _ @* pwhisker_left _ (pwhisker_left _ !ap1_pcompose_pinverse),
      refine !passoc^-1* @* _ @* !passoc @* !passoc,
      apply pwhisker_right,
      refine !ap1_pcompose^-1* @* _ @* !ap1_pcompose @* pwhisker_right _ !ap1_pcompose,
      apply ap1_phomotopy,
      exact fiber_sequence_phomotopy_loop_spaces k
    end

Definition pid_or_pinverse_right : forall (n : ℕ), loop_spaces n ->* loop_spaces n
  | 0     . !pid
  | 1     . !pid
  | 2     . !pid
  | (k+3) . Ω->(pid_or_pinverse_right k) o* pinverse

Definition pid_or_pinverse_left : forall (n : ℕ), loop_spaces n ->* loop_spaces n
  | 0     . pequiv.rfl
  | 1     . pequiv.rfl
  | 2     . pequiv.rfl
  | 3     . pequiv.rfl
  | 4     . pequiv.rfl
  | (k+5) . Ω->(pid_or_pinverse_left (k+2)) o* pinverse

Definition pid_or_pinverse_right_add3 (n : ℕ)
    : pid_or_pinverse_right (n + 3) = Ω->(pid_or_pinverse_right n) o* pinverse.
  by reflexivity

Definition pid_or_pinverse_left_add5 (n : ℕ)
    : pid_or_pinverse_left (n + 5) = Ω->(pid_or_pinverse_left (n+2)) o* pinverse.
  by reflexivity

Definition pid_or_pinverse_commute_right : forall (n : ℕ),
    loop_spaces_fun n ==* pid_or_pinverse_right n o* loop_spaces_fun n o* pid_or_pinverse (n + 1)
  | 0     . proof !pcompose_pid^-1* @* !pid_pcompose^-1* qed
  | 1     . proof !pcompose_pid^-1* @* !pid_pcompose^-1* qed
  | 2     . proof !pcompose_pid^-1* @* !pid_pcompose^-1* qed
  | (k+3).
    begin
      replace (k + 3 + 1) with (k + 4),
      rewrite [pid_or_pinverse_right_add3, loop_spaces_fun_add3] =>
      refine _ @* pwhisker_left _ (pwhisker_left _ !pid_or_pinverse_add4_rev^-1*),
      refine ap1_phomotopy (pid_or_pinverse_commute_right k) @* _,
      refine !ap1_pcompose @* _ @* !passoc^-1*,
      apply pwhisker_left,
      refine !ap1_pcompose @* _ @* !passoc @* !passoc,
      apply pwhisker_right,
      refine _ @* pwhisker_right _ !ap1_pcompose_pinverse,
      refine _ @* !passoc^-1*,
      refine !pcompose_pid^-1* @* pwhisker_left _ _,
      symmetry, apply pinverse_pinverse
    end

Definition pid_or_pinverse_commute_left : forall (n : ℕ),
    loop_spaces_fun n o* pid_or_pinverse_left (n + 1) ==* pid_or_pinverse n o* loop_spaces_fun n
  | 0     . proof !pcompose_pid @* !pid_pcompose^-1* qed
  | 1     . proof !pcompose_pid @* !pid_pcompose^-1* qed
  | 2     . proof !pcompose_pid @* !pid_pcompose^-1* qed
  | 3     . proof !pcompose_pid @* !pid_pcompose^-1* qed
  | (k+4).
    begin
      ...
      refine !passoc^-1* @* _ @* !passoc^-1*,
      refine _ @* pwhisker_left _ !ap1_pcompose_pinverse,
      refine _ @* !passoc,
      apply pwhisker_right,
      refine !ap1_pcompose^-1* @* _ @* !ap1_pcompose,
      exact ap1_phomotopy (pid_or_pinverse_commute_left (k+1))
    end

Definition LES_of_loop_spaces' : type_chain_complex +ℕ.
  transfer_type_chain_complex
    fiber_sequence
    (fun n => loop_spaces_fun n o* pid_or_pinverse (n + 1))
...
Definition fibration_sequence : fin 3 -> pType
  | (fin.mk 0 H)     . Y
  | (fin.mk 1 H)     . X
  | (fin.mk 2 H)     . pfiber f
  | (fin.mk (n+3) H) . begin exfalso, apply lt_le_antisymm H, apply le_add_left end

Definition loop_spaces2 : +3ℕ -> pType
  | (n, m) . Ω[n] (fibration_sequence m)

Definition loop_spaces2_add1 (n : ℕ) : forall (x : fin 3),
    loop_spaces2 (n+1, x) = loops (loop_spaces2 (n, x))
...
Definition loop_spaces_fun2 : forall , loop_spaces2 (S n) ->* loop_spaces2 n
  | (n, fin.mk 0 H) . proof Ω->[n] f qed
  | (n, fin.mk 1 H) . proof Ω->[n] (ppoint f) qed
  | (n, fin.mk 2 H) . proof Ω->[n] boundary_map o* loopn_succ_in Y n qed
...
Definition loop_spaces_fun2_add1_2 (n : ℕ) (H : 2 < succ 2)
  ...
  proof !ap1_pcompose^-1* qed

Definition nat_of_str {n : ℕ} : ℕ \* fin (succ n) -> ℕ.
  fun x => succ n * pr1 x + val (pr2 x)

Definition str_of_nat {n : ℕ} : ℕ -> ℕ \* fin (succ n).
  fun m => (m / (succ n), mk_mod n m)

Definition nat_of_str_3S [reducible]
    : forall (x : stratified +ℕ 2), nat_of_str x + 1 = nat_of_str (@S (stratified +ℕ 2) x)
  | (n, fin.mk 0 H) . by reflexivity
  | (n, fin.mk 1 H) . by reflexivity
  | (n, fin.mk 2 H) . by reflexivity
  | (n, fin.mk (k+3) H) . begin exfalso, apply lt_le_antisymm H, apply le_add_left end

Definition fin_prod_nat_equiv_nat (n : ℕ) : ℕ \* fin (succ n) <~> ℕ.
  equiv.MK nat_of_str str_of_nat
    abstract begin
      intro m, unfold [nat_of_str, str_of_nat, mk_mod],
      refine _ @ (eq_div_mul_add_mod m (succ n))^-1,
      rewrite [mul.comm]
    end end
    abstract begin
  ...
  (*
    note: in the followingDefinition the (n+1) case is 3 times the same,
    so maybe this can be simplified
  *)
Definition loop_spaces2_pequiv' : forall (n : ℕ) (x : fin (nat.succ 2)),
...
Definition loop_spaces2_pequiv : forall (x : +3ℕ),
...
Definition loop_spaces_fun2_phomotopy (x : +3ℕ) :
    loop_spaces2_pequiv x o* loop_spaces_fun (nat_of_str x) ==*
      (loop_spaces_fun2 x o* loop_spaces2_pequiv (S x))
    o* pcast (ap (loop_spaces) (nat_of_str_3S x)).
Proof.
    ...
    { (*k=0*)
      ...
      { refine !pid_pcompose @* _ @* !pcompose_pid^-1* @* !pcompose_pid^-1*,
        reflexivity},
      { refine _ @* !pcompose_pid^-1*,
        refine _ @* pwhisker_right _ !loop_spaces_fun2_add1_0^-1* =>
        refine !ap1_pcompose^-1* @* _ @* !ap1_pcompose, apply ap1_phomotopy,
    ...
    { (*k=1*)
      ...
      { refine !pid_pcompose @* _ @* !pcompose_pid^-1* @* !pcompose_pid^-1*,
        reflexivity},
      { refine _ @* !pcompose_pid^-1*,
        refine _ @* pwhisker_right _ !loop_spaces_fun2_add1_1^-1* =>
        refine !ap1_pcompose^-1* @* _ @* !ap1_pcompose, apply ap1_phomotopy,
        exact IH @* !pcompose_pid}},
    { (*k=2*)
      induction n with n IH,
      { refine !pid_pcompose @* _ @* !pcompose_pid^-1*,
        refine !pcompose_pid^-1* @* pconcat2 _ _,
        { exact (pcompose_pid (chain_complex.boundary_map f))^-1*},
        { refine !loop_pequiv_loop_rfl^-1* }},
      { refine _ @* !pcompose_pid^-1*,
        refine _ @* pwhisker_right _ !loop_spaces_fun2_add1_2^-1* =>
        refine !ap1_pcompose^-1* @* _ @* !ap1_pcompose, apply ap1_phomotopy,
        exact IH @* !pcompose_pid}},
Defined.

Definition LES_of_loop_spaces2 : type_chain_complex +3ℕ.
  transfer_type_chain_complex2
    ...
    begin
      ...
      esimp, exact ap010 cast !ap_compose^-1 x
    end
...
Definition LES_of_homotopy_groups' : chain_complex +3ℕ.
...
Definition homotopy_groups : +3ℕ -> Set*.
  fun nm => forall [nm.1] (fibration_sequence nm.2)

Definition homotopy_groups_pequiv_loop_spaces2 [reducible]
    : forall (n : +3ℕ), ptrunc 0 (loop_spaces2 n) <~>* homotopy_groups n
  | (n, fin.mk 0 H) . by reflexivity
  | (n, fin.mk 1 H) . by reflexivity
  | (n, fin.mk 2 H) . by reflexivity
  | (n, fin.mk (k+3) H) . begin exfalso, apply lt_le_antisymm H, apply le_add_left end

Definition homotopy_groups_fun : forall , homotopy_groups (S n) ->* homotopy_groups n
  | (n, fin.mk 0 H) . proof forall ->[n] f qed
  | (n, fin.mk 1 H) . proof forall ->[n] (ppoint f) qed
  | (n, fin.mk 2 H).
    proof forall ->[n] boundary_map o* homotopy_group_succ_in Y n qed
  | (n, fin.mk (k+3) H) . begin exfalso, apply lt_le_antisymm H, apply le_add_left end

Definition homotopy_groups_fun_phomotopy_loop_spaces_fun2 [reducible]
    : forall (n : +3ℕ), homotopy_groups_pequiv_loop_spaces2 n o* ptrunc_functor 0 (loop_spaces_fun2 n) ==*
      homotopy_groups_fun n o* homotopy_groups_pequiv_loop_spaces2 (S n)
  | (n, fin.mk 0 H) . by reflexivity
  | (n, fin.mk 1 H) . by reflexivity
  | (n, fin.mk 2 H).
    begin
...
Defined.
  ...
  (* the carrier of the fiber sequence isDefinitionally what we want (as pointed sets) *)
  example : LES_of_homotopy_groups (str_of_nat 6)  = forall [2] Y          :> Set* . by reflexivity
  example : LES_of_homotopy_groups (str_of_nat 7)  = forall [2] X          :> Set* . by reflexivity
  example : LES_of_homotopy_groups (str_of_nat 8)  = forall [2] (pfiber f) :> Set* . by reflexivity
  example : LES_of_homotopy_groups (str_of_nat 9)  = forall [3] Y          :> Set* . by reflexivity
  example : LES_of_homotopy_groups (str_of_nat 10) = forall [3] X          :> Set* . by reflexivity
  example : LES_of_homotopy_groups (str_of_nat 11) = forall [3] (pfiber f) :> Set* . by reflexivity

Definition LES_of_homotopy_groups_0 : LES_of_homotopy_groups (n, 0) = forall [n] Y.
  by reflexivity
Definition LES_of_homotopy_groups_1 : LES_of_homotopy_groups (n, 1) = forall [n] X.
  by reflexivity
Definition LES_of_homotopy_groups_2 : LES_of_homotopy_groups (n, 2) = forall [n] (pfiber f).
  by reflexivity

  (*
    the functions of the fiber sequence isDefinitionally what we want (as pointed function).
  *)

Definition LES_of_homotopy_groups_fun_0 :
    cc_to_fn LES_of_homotopy_groups (n, 0) = forall ->[n] f.
  by reflexivity
Definition LES_of_homotopy_groups_fun_1 :
    cc_to_fn LES_of_homotopy_groups (n, 1) = forall ->[n] (ppoint f).
  by reflexivity
Definition LES_of_homotopy_groups_fun_2 : cc_to_fn LES_of_homotopy_groups (n => 2) =
    forall ->[n] boundary_map o* homotopy_group_succ_in Y n.
  by reflexivity
...
Definition Group_LES_of_homotopy_groups (n : +3ℕ) : Group.{u}.
  forall g[n.1+1] (fibration_sequence n.2)

Definition AbGroup_LES_of_homotopy_groups (n : +3ℕ) : AbGroup.{u}.
  forall ag[n.1+2] (fibration_sequence n.2)

Definition homomorphism_LES_of_homotopy_groups_fun : forall ,
  ...
  | (k, fin.mk 2 H).
    begin
      ...
      refine homomorphism.struct ((forall ->g[k+1] boundary_map) og ghomotopy_group_succ_in Y k),
      end end
    end
...
Definition LES_isomorphism_of_trivial_cod (n : ℕ) [H : is_succ n]
    (HX1 : is_contr (forall g[n] Y)) (HX2 : is_contr (forall g[n+1] Y)) : forall g[n] (pfiber f) <~>g forall g[n] X.
Proof.
    induction H with n,
...
Definition LES_isomorphism_of_trivial_dom (n : ℕ) [H : is_succ n]
    (HX1 : is_contr (forall g[n] X)) (HX2 : is_contr (forall g[n+1] X)) : forall g[n+1] Y <~>g forall g[n] (pfiber f).
Proof.
    induction H with n,
...
Definition LES_isomorphism_of_trivial_pfiber (n : ℕ)
    (HX1 : is_contr (forall [n] (pfiber f))) (HX2 : is_contr (forall g[n+1] (pfiber f))) : forall g[n+1] X <~>g forall g[n+1] Y.
Proof.
...
Definition fibration_sequence_car : +3ℕ -> pType
  | (n, fin.mk 0 H) . Ω[n] Y
  | (n, fin.mk 1 H) . Ω[n] X
  | (n, fin.mk k H) . Ω[n] F

Definition fibration_sequence_fun
    : forall (n : +3ℕ), fibration_sequence_car (S n) ->* fibration_sequence_car n
  | (n, fin.mk 0 H) . proof Ω->[n] f qed
  | (n, fin.mk 1 H) . proof Ω->[n] g qed
  | (n, fin.mk 2 H) . proof Ω->[n] (e o* boundary_map f) o* loopn_succ_in Y n qed
  | (n, fin.mk (k+3) H) . begin exfalso, apply lt_le_antisymm H, apply le_add_left end

Definition fibration_sequence_pequiv : forall (x : +3ℕ),
...
Definition fibration_sequence_fun_phomotopy : forall ,
    fibration_sequence_pequiv x o* loop_spaces_fun2 f x ==*
      (fibration_sequence_fun x o* fibration_sequence_pequiv (S x))
  | (n, fin.mk 0 H) . by reflexivity
  | (n, fin.mk 1 H).
    begin refine !pid_pcompose @* _, refine apn_phomotopy n p @* _,
      refine !apn_pcompose @* _, reflexivity end
  | (n, fin.mk 2 H) . begin refine !passoc^-1* @* _ @* !pcompose_pid^-1*, apply pwhisker_right,
      refine _ @* !apn_pcompose^-1*, reflexivity end
  | (n, fin.mk (k+3) H) . begin exfalso, apply lt_le_antisymm H, apply le_add_left end

Definition type_LES_fibration_sequence : type_chain_complex +3ℕ.
  transfer_type_chain_complex
...
Definition LES_fibration_sequence : chain_complex +3ℕ.
