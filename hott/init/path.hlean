...
Definition rec_unbased {P : forall (a b : A), (a = b) -> Type}
  (H : forall (a : A), P a a idp) {a b : A} (p : a = b) : P a b p.
  eq.rec (H a) p

Definition rec_on_unbased {P : forall (a b : A), (a = b) -> Type}
  {a b : A} (p : a = b) (H : forall (a : A), P a a idp) : P a b p.
  eq.rec (H a) p

  (* Concatenation and inverse *)

Definition concat [trans] (p : x = y) (q : y = z) : x = z.
  by induction q; exact p

Definition inverse [symm] (p : x = y) : y = x.
  by induction p; reflexivity

  infix   @  . concat
  postfix ^-1 . inverse
  --a second notation for the inverse, which is not overloaded
  postfix [parsing_only] `^-1ᵖ`:std.prec.max_plus . inverse
...
Definition con.right_inv (p : x = y) : p @ p^-1 = idp.
  by induction p; reflexivity

  (* The left inverse law. *)
Definition con.left_inv (p : x = y) : p^-1 @ p = idp.
  by induction p; reflexivity

  (* Several auxiliaryDefinitions about canceling inverses across associativity. These are somewhat
  redundant, following from earlierDefinitions. *)

Definition inv_con_cancel_left (p : x = y) (q : y = z) : p^-1 @ (p @ q) = q.
  by induction q; induction p; reflexivity

Definition con_inv_cancel_left (p : x = y) (q : x = z) : p @ (p^-1 @ q) = q.
  by induction q; induction p; reflexivity

Definition con_inv_cancel_right (p : x = y) (q : y = z) : (p @ q) @ q^-1 = p.
  by induction q; reflexivity

Definition inv_con_cancel_right (p : x = z) (q : y = z) : (p @ q^-1) @ q = p.
  by induction q; reflexivity

  (* Inverse distributes over concatenation *)
Definition con_inv (p : x = y) (q : y = z) : (p @ q)^-1 = q^-1 @ p^-1.
  by induction q; induction p; reflexivity

Definition inv_con_inv_left (p : y = x) (q : y = z) : (p^-1 @ q)^-1 = q^-1 @ p.
  by induction q; induction p; reflexivity

Definition inv_con_inv_right (p : x = y) (q : z = y) : (p @ q^-1)^-1 = q @ p^-1.
  by induction q; induction p; reflexivity

Definition inv_con_inv_inv (p : y = x) (q : z = y) : (p^-1 @ q^-1)^-1 = q @ p.
  by induction q; induction p; reflexivity

  (* Inverse is an involution. *)
Definition inv_inv (p : x = y) : p^-1^-1 = p.
  by induction p; reflexivity

  (* auxiliaryDefinition used by 'cases' tactic *)
Definition elim_inv_inv {A : Type} {a b : A} {C : a = b -> Type}
  (H₁ : a = b) (H₂ : C (H₁^-1^-1)) : C H₁.
  eq.rec_on (inv_inv H₁) H₂

Definition eq.rec_symm {A : Type} {a₀ : A} {P : forall (a₁), a₁ = a₀ -> Type}
...
Definition con_eq_of_eq_inv_con {p : x = z} {q : y = z} {r : y = x} :
  p = r^-1 @ q -> r @ p = q.
Proof.
  induction r, intro h, exact (concat_1p _) @ h @ (concat_1p _)
Defined.
...
Definition inv_con_eq_of_eq_con {p : x = z} {q : y = z} {r : x = y} :
  p = r @ q -> r^-1 @ p = q.
  by induction r; intro h; exact (concat_1p _) @ h @ (concat_1p _)
...
Definition eq_con_of_inv_con_eq {p : x = z} {q : y = z} {r : y = x} :
  r^-1 @ q = p -> q = r @ p.
  by induction r; intro h; exact (concat_1p _)^-1 @ h @ (concat_1p _)^-1
...
Definition eq_of_con_inv_eq_idp {p q : x = y} : p @ q^-1 = idp -> p = q.
  by induction q; exact id

Definition eq_of_inv_con_eq_idp {p q : x = y} : q^-1 @ p = idp -> p = q.
  by induction q; intro h; exact (concat_1p _)^-1 @ h

Definition eq_inv_of_con_eq_idp' {p : x = y} {q : y = x} : p @ q = idp -> p = q^-1.
  by induction q; exact id

Definition eq_inv_of_con_eq_idp {p : x = y} {q : y = x} : q @ p = idp -> p = q^-1.
  by induction q; intro h; exact (concat_1p _)^-1 @ h

Definition eq_of_idp_eq_inv_con {p q : x = y} : idp = p^-1 @ q -> p = q.
  by induction p; intro h; exact h @ (concat_1p _)

Definition eq_of_idp_eq_con_inv {p q : x = y} : idp = q @ p^-1 -> p = q.
  by induction p; exact id

Definition inv_eq_of_idp_eq_con {p : x = y} {q : y = x} : idp = q @ p -> p^-1 = q.
  by induction p; exact id

Definition inv_eq_of_idp_eq_con' {p : x = y} {q : y = x} : idp = p @ q -> p^-1 = q.
  by induction p; intro h; exact h @ (concat_1p _)

Definition con_inv_eq_idp {p q : x = y} (r : p = q) : p @ q^-1 = idp.
  by cases r; apply con.right_inv

Definition inv_con_eq_idp {p q : x = y} (r : p = q) : q^-1 @ p = idp.
  by cases r; apply con.left_inv

Definition con_eq_idp {p : x = y} {q : y = x} (r : p = q^-1) : p @ q = idp.
  by cases q; exact r

Definition idp_eq_inv_con {p q : x = y} (r : p = q) : idp = p^-1 @ q.
  by cases r; exact (con_Vp _)^-1

Definition idp_eq_con_inv {p q : x = y} (r : p = q) : idp = q @ p^-1.
  by cases r; exact (con_pV _)^-1

Definition idp_eq_con {p : x = y} {q : y = x} (r : p^-1 = q) : idp = q @ p.
  by cases p; exact r
...
Definition eq_idp_of_con_left {p : x = x} {q : y = x} (r : q @ p = q) : p = idp.
  by cases q; exact (idp_con p)^-1 @ r
...
Definition transport [subst] (P : A -> Type) {x y : A} (p : x = y)
  (u : P x) : P y.
...
Definition cast_def [unfold_full] {A B : Type} (p : A = B) (a : A)
  : cast p a = p # a.
  idp

Definition tr_rev (P : A -> Type) {x y : A} (p : x = y) (u : P y) : P x.
  p^-1 # u
...
Definition homotopy (f g : forall x, P x) : Type.
  forall x : A, f x = g x

  infix == . homotopy

  protectedDefinition homotopy.refl [refl] [unfold_full] (f : forall x, P x) : f == f.
  fun x => idp

  protectedDefinition homotopy.rfl [unfold_full] {f : forall x, P x} : f == f.
  homotopy.refl f

  protectedDefinition homotopy.symm [symm] [unfold_full] {f g : forall x, P x} (H : f == g)
  : g == f.
  fun x => (H x)^-1

  protectedDefinition homotopy.trans [trans] [unfold_full] {f g h : forall x, P x}
  (H1 : f == g) (H2 : g == h) : f == h.
  fun x => H1 x @ H2 x

  infix ` @hty `:75 . homotopy.trans
  postfix `^-1ʰᵗʸ`:(max+1) . homotopy.symm
...
Definition compose_id (f : A -> B) : f o id == f.
  by reflexivity

Definition id_compose (f : A -> B) : id o f == f.
  by reflexivity

Definition compose2 {A B C : Type} {g g' : B -> C} {f f' : A -> B}
  (p : g == g') (q : f == f') : g o f == g' o f'.
  hwhisker_right f p @hty hwhisker_left g' q

Definition hassoc {A B C D : Type} (h : C -> D) (g : B -> C) (f : A -> B) : (h o g) o f == h o (g o f).
  fun a => idp

Definition homotopy_of_eq {f g : forall x, P x} (H1 : f = g) : f == g.
  H1 # homotopy.refl f

Definition apd10 {f g : forall x, P x} (H : f = g) : f == g.
  fun x => by induction H; reflexivity

  --the nextDefinition is useful if you want to write "apply (apd10' a)"
Definition apd10' {f g : forall x, P x} (a : A) (H : f = g) : f a = g a.
  by induction H; reflexivity

  --apd10 is also ap evaluation
Definition apd10_eq_ap_eval {f g : forall x, P x} (H : f = g) (a : A)
  : apd10 H a = ap (fun s : forall , P x, s a) H.
  by induction H; reflexivity
...
Definition apdt (f : forall a, P a) {x y : A} (p : x = y) : p # f x = f y.
  by induction p; reflexivity

Definition ap011 (f : A -> B -> C) (Ha : a = a') (Hb : b = b') : f a b = f a' b'.
  by cases Ha; exact ap (f a) Hb

  (* MoreDefinitions for moving things around in equations *)
...
Definition tr_diag_eq_tr_tr {A : Type} (P : A -> A -> Type) {x y : A} (p : x = y) (a : P x x) :
  transport (fun x => P x x) p a = transport (fun x => P _ x) p (transport (fun x => P x _) p a).
  by induction p; reflexivity
...
Definition ap_inv' (f : A -> B) {x y : A} (p : x = y) : (ap f p)^-1 = ap f p^-1.
  by induction p; reflexivity
...
Definition ap_constant (p : x = y) (z : B) : ap (fun u => z) p = idp.
  by induction p; reflexivity
...
Definition ap_con_eq_con {f : A -> A} (p : forall x, f x = x) {x y : A} (q : x = y) :
  ap f q @ p y = p x @ q.
  by induction q; apply idp_con

Definition con_ap_eq_con {f : A -> A} (p : forall x, x = f x) {x y : A} (q : x = y) :
  p x @ ap f q =  q @ p y.
  by induction q; exact (concat_1p _)^-1

  (* Naturality of [ap] with constant function *)
Definition ap_con_eq {f : A -> B} {b : B} (p : forall x, f x = b) {x y : A} (q : x = y) :
  ap f q @ p y = p x.
  by induction q; apply idp_con
...
Proof. cases q, exact idp end

Definition con_ap_con_eq_con_con' {g : A -> A} (p : id == g) {x y : A} (q : x = y)
  {z : A} (s : g y = z) :
  p x @ (ap g q @ s) = q @ (p y @ s).
  by induction s; induction q; exact (concat_1p _)^-1

  (* Action of [apd10] and [ap10] on paths *)

  (* Application of paths between functions preserves the groupoid structure *)

Definition apd10_idp (f : forall x, P x) (x : A) : apd10 (refl f) x = idp . idp

Definition apd10_con {f f' f'' : forall x, P x} (h : f = f') (h' : f' = f'') (x : A) :
  apd10 (h @ h') x = apd10 h x @ apd10 h' x.
  by induction h; induction h'; reflexivity

Definition apd10_inv {f g : forall x : A, P x} (h : f = g) (x : A) :
  apd10 h^-1 x = (apd10 h x)^-1.
  by induction h; reflexivity
...
Definition ap10_inv {f g : A -> B} (h : f = g) (x : A) : ap10 h^-1 x = (ap10 h x)^-1.
  apd10_inv h x
...
Definition ap_eq_ap011_left (f : A -> B -> C) (Ha : a = a') (b : B) :
  ap (fun a => f a b) Ha = ap011 f Ha idp.
...
Definition ap_ap011 {A B C D : Type} (g : C -> D) (f : A -> B -> C) {a a' : A} {b b' : B}
  (p : a = a') (q : b = b') : ap g (ap011 f p q) = ap011 (fun a b => g (f a b)) p q.
Proof.
  induction p, exact (ap_compose g (f a) q)^-1
Defined.
...
Definition tr_inv_tr {P : A -> Type} {x y : A} (p : x = y) (z : P y) :
  p # p^-1 # z = z.
  (con_tr p^-1 p z)^-1 @ ap (fun r => transport P r z) (con.left_inv p)

Definition inv_tr_tr {P : A -> Type} {x y : A} (p : x = y) (z : P x) :
  p^-1 # p # z = z.
  (con_tr p p^-1 z)^-1 @ ap (fun r => transport P r z) (con.right_inv p)

Definition cast_cast_inv {A : Type} {P : A -> Type} {x y : A} (p : x = y) (z : P y) :
  cast (ap P p) (cast (ap P p^-1) z) = z.
  by induction p; reflexivity

Definition cast_inv_cast {A : Type} {P : A -> Type} {x y : A} (p : x = y) (z : P x) :
  cast (ap P p^-1) (cast (ap P p) z) = z.
  by induction p; reflexivity

Definition fn_tr_eq_tr_fn {P Q : A -> Type} {x y : A} (p : x = y) (f : forall x, P x -> Q x) (z : P x) :
  f y (p # z) = p # f x z.
  by induction p; reflexivity

Definition fn_cast_eq_cast_fn {A : Type} {P Q : A -> Type} {x y : A} (p : x = y)
  (f : forall x, P x -> Q x) (z : P x) : f y (cast (ap P p) z) = cast (ap Q p) (f x z).
  by induction p; reflexivity

Definition con_con_tr {P : A -> Type}
  {x y z w : A} (p : x = y) (q : y = z) (r : z = w) (u : P x) :
  ap (fun e => e # u) (con.assoc' p q r) @ (con_tr (p @ q) r u) @
...
Definition apdt_idp (x : A) (f : forall x, P x) : apdt f idp = idp :> (f x = f x) . idp

Definition apdt_con (f : forall x, P x) {x y z : A} (p : x = y) (q : y = z)
  : apdt f (p @ q) = con_tr p q (f x) @ ap (transport P q) (apdt f p) @ apdt f q.
  by cases p; cases q; apply idp

Definition apdt_inv (f : forall x, P x) {x y : A} (p : x = y)
  : apdt f p^-1 = (eq_inv_tr_of_tr_eq (apdt f p))^-1.
  by cases p; apply idp

  (* Dependent transport in a doubly dependent type. *)
  (* This is a special case of transporto in init.pathover *)
Definition transportD {P : A -> Type} (Q : forall a, P a -> Type)
...
Definition tr2_inv (Q : A -> Type) {x y : A} {p q : x = y} (r : p = q) (z : Q x) :
  transport2 Q r^-1 z = (transport2 Q r z)^-1.
  by induction r; reflexivity

Definition transportD2 {B C : A -> Type} (D : forall (a:A), B a -> C a -> Type)
...
Definition ap_tr_con_tr2 (P : A -> Type) {x y : A} {p q : x = y} {z w : P x} (r : p = q)
  ...
  by induction r; exact (concat_1p _)^-1
...
Definition tr_constant (p : x = y) (z : B) : transport (fun x => B) p z = z.
  by induction p; reflexivity

Definition tr2_constant {p q : x = y} (r : p = q) (z : B) :
  tr_constant p z = transport2 (fun u => B) r z @ tr_constant q z.
  by induction r; exact (concat_1p _)^-1
...
Definition tr_ap (P : B -> Type) (f : A -> B) (p : x = y) (z : P (f x)) :
  transport P (ap f p) z = transport (P o f) p z.
  (tr_compose P f p z)^-1

Definition ap_precompose (f : A -> B) (g g' : B -> C) (p : g = g') :
  ap (fun h => h o f) p = transport (fun h : B -> C => g o f = h o f) p idp.
  by induction p; reflexivity

Definition apd10_ap_precompose (f : A -> B) (g g' : B -> C) (p : g = g') :
  apd10 (ap (fun h : B -> C => h o f) p) = fun a => apd10 p (f a).
  by induction p; reflexivity

Definition apd10_ap_precompose_dependent {C : B -> Type}
  (f : A -> B) {g g' : forall b : B, C b} (p : g = g')
  : apd10 (ap (fun (h : (forall , C b))(a : A), h (f a)) p) = fun a => apd10 p (f a).
  by induction p; reflexivity

Definition apd10_ap_postcompose (f : B -> C) (g g' : A -> B) (p : g = g') :
  apd10 (ap (fun h : A -> B => f o h) p) = fun a => ap f (apd10 p a).
  by induction p; reflexivity
...
Definition inverse2 {p q : x = y} (h : p = q) : p^-1 = q^-1.
  ap inverse h
...
Definition whisker_right {p q : x = y} (r : y = z) (h : p = q) : p @ r = q @ r.
  h ◾ idp

  (* Unwhiskering, a.k.a. cancelling *)

Definition cancel_left {x y z : A} (p : x = y) {q r : y = z} : (p @ q = p @ r) -> (q = r).
  fun s => !inv_con_cancel_left^-1 @ whisker_left p^-1 s @ !inv_con_cancel_left

Definition cancel_right {x y z : A} {p q : x = y} (r : y = z) : (p @ r = q @ r) -> (p = q).
  fun s => !con_inv_cancel_right^-1 @ whisker_right r^-1 s @ !con_inv_cancel_right
...
Definition whisker_left_idp {p q : x = y} (h : p = q) :
  (idp_con p)^-1 @ whisker_left idp h @ idp_con q = h.
  by induction h; induction p; reflexivity
...
Definition inv2_con2 {p p' : x = y} (h : p = p')
  : h⁻² ◾ h = con.left_inv p @ (con.left_inv p')^-1.
  by induction h; induction p; reflexivity
...
Proof.
  refine (whisker_right_idp p ◾ whisker_left_idp2 q)^-1 @ _,
...
Definition inv_eq_inv2 (p : idp = idp :> a = a) : p^-1 = p⁻².
Proof.
  apply eq.cancel_right p,
  refine (con_Vp _) @ _,
  refine _ @ !con_eq_con2^-1,
  exact !inv2_con2^-1,
Defined.

  (* The action of functions on 2-dimensional paths *)
Definition ap02 (f : A -> B) {x y : A} {p q : x = y} (r : p = q)
  : ap f p = ap f q.
  ap (ap f) r
...
Definition apdt02 {p q : x = y} (f : forall x, P x) (r : p = q) :
...
(*
  ...
  namespace because ^-1ʰ is also used as the inverse of a homomorphism
*)

open eq
namespace homotopy
  infix ` @h `:75 . homotopy.trans
  postfix `^-1ʰ`:(max+1) . homotopy.symm
Defined. homotopy
