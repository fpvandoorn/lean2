...
Proof of theDefinition that (is_trunc n A) is a mere proposition
...
Definition is_contr.sigma_char (A : Type) :
  (Σ (center : A), forall (a : A), center = a) <~> (is_contr A).
Proof.
  fapply equiv.MK,
...
Definition is_trunc.pi_char (n : trunc_index) (A : Type) :
  (forall (x y : A), is_trunc n (x = y)) <~> (is_trunc (n .+1) A).
Proof.
  fapply equiv.MK,
...
Definition is_prop_is_trunc (n : trunc_index) :
  forall (A : Type), is_prop (is_trunc n A).
Proof.
  induction n,
  { intro A,
