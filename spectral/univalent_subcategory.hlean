...
Definition eq_equiv_iso_of_fully_faithful : a = b <~> a ≅ b.
  equiv.mk !ap !p                              @e equiv.mk iso_of_eq !iso_of_path_equiv     @e equiv.symm !iso_equiv_F_iso_F
...
Definition precategory_Group.{u} [instance] : precategory.{u+1 u} Group.
Proof.
  fapply precategory.mk,
  { exact fun G H => G ->g H },
  { exact _ },
  { exact fun G H K ψ φ => ψ og φ },
  { exact fun G => gid G },
...
Definition precategory_AbGroup.{u} [instance] : precategory.{u+1 u} AbGroup.
Proof.
  fapply precategory.mk,
  { exact fun G H => G ->g H },
  { exact _ },
...
Proof.
  ...
  fapply trunctype.mk,
  { exact is_set A \* (forall a, m a o = a) \* (forall a, m o a = a) \* (forall a b c, m (m a b) c = m a (m b c)) \*
  (forall a, m (i a) a = o) },
  { apply is_trunc_of_imp_is_trunc, intro v, induction v with H v,
  have is_prop (forall a, m a o = a), from _,
  have is_prop (forall a, m o a = a), from _,
  have is_prop (forall a b c, m (m a b) c = m a (m b c)), from _,
  have is_prop (forall a, m (i a) a = o), from _,
  apply is_trunc_prod }
Defined.
...
Proof.
  ...
  fapply trunctype.mk,
  { exact is_set A \* (forall a, m a o = a) \* (forall a, m o a = a) \* (forall a b c, m (m a b) c = m a (m b c)) \*
  (forall a, m (i a) a = o) \* (forall a b, m a b = m b a)},
  { apply is_trunc_of_imp_is_trunc, intro v, induction v with H v,
  have is_prop (forall a, m a o = a), from _,
  have is_prop (forall a, m o a = a), from _,
  have is_prop (forall a b c, m (m a b) c = m a (m b c)), from _,
  have is_prop (forall a, m (i a) a = o), from _,
  have is_prop (forall a b, m a b = m b a), from _,
  apply is_trunc_prod }
Defined.

Definition AbGroup_sigma.{u} : AbGroup.{u} <~> Σ A : Type.{u}, ab_group A.
Proof. repeat (assumption | induction a with a b | intro a | fconstructor) end

Definition Group_sigma.{u} : Group.{u} <~> Σ A : Type.{u}, group A.
Proof.
fconstructor, exact fun a => dpair (Group.carrier a) (Group.struct' a),
...
sigma_equiv_sigma_right (fun a => !sigma.equiv_prod^-1ᵉ) @e !sigma_assoc_equiv'
...
Definition inv_eq_of_mul_eq {A : Type} (G H : group A) (p : @mul A G ~2 @mul A H) :
  @inv A G == @inv A H.
Proof.
...
Definition one_eq_of_mul_eq {A : Type} (G H : group A)
...
Proof.
  ...
  exact (Hh2 G1)^-1 @ (p H1 G1)^-1 @ Gh3 H1,
Defined.
Defined.
...
Definition Group_eq_equiv_lemma2 {A : Type} {m m' : A -> A -> A} {i i' : A -> A} {o o' : A}
...
Definition Group_eq_equiv_lemma {G H : Group}
...
Proof.
  ...
  exact !isomorphism.sigma_char^-1ᵉ
Defined.
...
Proof.
  ...
  intro g, refine to_fun_Group_eq_equiv p g @ r g @ (to_fun_Group_eq_equiv q g)^-1 =>
Defined.

Definition Group_eq_equiv_Group_iso (G₁ G₂ : Group) : G₁ = G₂ <~> G₁ ≅ G₂.
  Group_eq_equiv G₁ G₂ @e (Group_iso_equiv G₁ G₂)^-1ᵉ
...
Definition AbGroup_to_Group : functor (Precategory.mk AbGroup _)
  (Category.mk Group category_Group)
. mk (fun x : AbGroup => (x : Group)) (fun a b x => x) (fun x => rfl) begin intros, reflexivity end
...
Definition ab_group_to_group_homot (A : Type) :
  @ab_group_to_group A == group_comm_to_group A o ab_group_equiv_group_comm A.
Proof. intro, induction x, reflexivity end
...
Definition is_embedding_total_of_is_embedding_fiber {A} {B C : A -> Type} {f : forall a, B a -> C a}
...
exact is_trunc_equiv_closed -1 (H^-1ᵉ) _
Defined.

Definition AbGroup_to_Group_homot : AbGroup_to_Group == Group_sigma^-1 o total ab_group_to_group o AbGroup_sigma.
Proof. intro g, induction g, reflexivity end
...
Definition Grp.{u} : Category . category.Mk Group.{u} category_Group
Definition AbGrp : Category . category.Mk AbGroup category_AbGroup
