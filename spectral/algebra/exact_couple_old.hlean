...
(* ThisDefinition needs to be moved to exactness.hlean. However we had trouble doing so. Please help. *)
...
Definition is_differential {B : AbGroup} (d : B ->g B) . forall (b:B), d (d b) = 1
...
Definition diff_im_in_ker {B : AbGroup} (d : B ->g B) (H : is_differential d) : forall (b : B), image_subgroup d b -> kernel_subgroup d b.
Proof.
  intro b p,
...
Proof.
  ...
  refine  _ @ ap k^-1ᵍ k3 @ _ ,
  esimp,
  exact (to_left_inv (equiv_of_isomorphism k) ((SES.g ses) b))^-1,
  exact to_respect_one k^-1ᵍ
Defined.
...
Proof.
  ...
  exact (homology_iso_ugly d H)^-1ᵍ
Defined.
...
Definition differential {A B : AbGroup} (EC : exact_couple A B) : B ->g B.
  (exact_couple.j EC) og (exact_couple.k EC)
...
Definition derived_couple_i : derived_couple_A ->g derived_couple_A.
  (image_lift (exact_couple.i EC)) og (image_incl (exact_couple.i EC))
...
Definition subgroup_iso_exact_at_A_triangle : ab_kernel_incl i == ab_image_incl k og subgroup_iso_exact_at_A.
Proof.
...
Definition subgroup_homom_ker_to_im : ab_kernel i ->g ab_image d.
  (image_homomorphism k j) og subgroup_iso_exact_at_A

open eq

Definition left_square_derived_ses_aux : j_factor og ab_image_incl k == (SES.f (SES_of_differential d H)) og (image_homomorphism k j).
Proof.
  intro x,
...
Definition left_square_derived_ses : j_factor og (ab_kernel_incl i) == (SES.f (SES_of_differential d H)) og subgroup_homom_ker_to_im.
Proof.
  intro x,
...
Definition derived_couple_j_unique :
  is_contr (Σ hC, group_fun (hC og SES.g SES_of_exact_couple_at_i) == group_fun
  (SES.g (SES_of_differential d H) og j_factor)).
...
Definition derived_couple_j_htpy : group_fun (derived_couple_j og SES.g SES_of_exact_couple_at_i) == group_fun
  (SES.g (SES_of_differential d H) og j_factor).
Proof.
...
Proof.
  ...
  exact k og ab_kernel_incl d,
  intro p, induction p with b p, exact p,
Defined.

Definition k_restrict : ab_kernel d ->g derived_couple_A.
  subgroup_iso_exact_kerj_imi og k_restrict_aux

Definition k_restrict_square_left : k_restrict og (SES.f (SES_of_differential d H)) == fun x => 1.
Proof.
  intro x,
...
Definition derived_couple_k_unique :   is_contr
  (Σ hC, group_fun (hC og SES.g (SES_of_differential d H)) == group_fun
  (SES.g SES_im_i_trivial og k_restrict))
 .
...
Definition derived_couple_k_htpy : group_fun (derived_couple_k og SES.g (SES_of_differential d H)) == group_fun
  (SES.g (SES_im_i_trivial) og k_restrict).
Proof.
