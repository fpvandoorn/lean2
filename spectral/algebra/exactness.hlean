...
structure is_exact_t {A B : Type} {C : pType} (f : A -> B) (g : B -> C).
  ( im_in_ker : forall (a:A), g (f a) = (point _))
  ( ker_in_im : forall (b:B), (g b = (point _)) -> fiber f b)

structure is_exact {A B : Type} {C : pType} (f : A -> B) (g : B -> C).
  ( im_in_ker : forall (a:A), g (f a) = (point _))
  ( ker_in_im : forall (b:B), (g b = (point _)) -> image f b)
...
Definition is_exact_g.mk {A B C : Group} {f : A ->g B} {g : B ->g C}
  (H₁ : forall a, g (f a) = 1) (H₂ : forall b, g b = 1 -> image f b) : is_exact_g f g.
is_exact.mk H₁ H₂

Definition is_exact.im_in_ker2 {A B : Type} {C : Set*} {f : A -> B} {g : B -> C} (H : is_exact f g)
  {b : B} (h : image f b) : g b = (point _).
Proof.
  induction h with a p, exact ap g p^-1 @ is_exact.im_in_ker H a
Defined.

Definition is_exact_homotopy {A B : Type} {C : pType} {f f' : A -> B} {g g' : B -> C}
...
Definition is_exact_trunc_functor {A B : Type} {C : pType} {f : A -> B} {g : B -> C}
...
Definition is_contr_middle_of_is_exact {A B : Type} {C : pType} {f : A -> B} {g : B -> C}
  (H : is_exact f g) [is_contr A] [is_set B] [is_contr C] : is_contr B.
Proof.
  apply is_contr.mk (f (point _)),
  intro b,
  induction is_exact.ker_in_im H b !is_prop.elim,
  exact ap f !is_prop.elim @ p
Defined.

Definition is_surjective_of_is_exact_of_is_contr {A B : Type} {C : pType} {f : A -> B} {g : B -> C}
  (H : is_exact f g) [is_contr C] : is_surjective f.
fun b => is_exact.ker_in_im H b !is_prop.elim
...
Proof.
  ...
  exact q^-1 @ ap f !is_prop.elim @ to_respect_one f
Defined.

Definition map_left_of_is_exact {G₃' G₃ G₂ : Type} {G₁ : pType}
...
Definition map_left_of_is_exact_compute {G₃' G₃ G₂ : Type} {G₁ : pType}
  {g : G₃ -> G₂} {g' : G₃' -> G₂} {f : G₂ -> G₁} (H1 : is_exact g f) (H2 : is_exact g' f)
  (Hg' : is_embedding g') (a : G₃) : g' (map_left_of_is_exact H1 H2 Hg' a) = g a.
@point_eq _ _ g' _ _

Definition map_left_of_is_exact_compose {G₃'' G₃' G₃ G₂ : Type} {G₁ : pType}
...
Proof.
  ...
  refine !map_left_of_is_exact_compute @ _ @ !map_left_of_is_exact_compute^-1,
  exact map_left_of_is_exact_compute H1 H2 Hg' a
Defined.

Definition map_left_of_is_exact_id {G₃ G₂ : Type} {G₁ : pType}
...
Definition map_left_of_is_exact_homotopy {G₃' G₃ G₂ : Type} {G₁ : pType}
...
Proof.
  intro a,
  refine @is_injective_of_is_embedding _ _ g' _ _ _ _,
  exact !map_left_of_is_exact_compute @ (!p @ !map_left_of_is_exact_compute)^-1,
Defined.
...
Proof.
  ...
  (to_respect_mul g' _ _ @ ap011 mul !point_eq !point_eq)^-1 }
Defined.
...
structure is_short_exact {A B : Type} {C : pType} (f : A -> B) (g : B -> C).
  (is_emb    : is_embedding f)
  (im_in_ker : forall (a:A), g (f a) = (point _))
  (ker_in_im : forall (b:B), (g b = (point _)) -> image f b)
  (is_surj   : is_surjective g)

structure is_short_exact_t {A B : Type} {C : pType} (f : A -> B) (g : B -> C).
  (is_emb    : is_embedding f)
  (im_in_ker : forall (a:A), g (f a) = (point _))
  (ker_in_im : forall (b:B), (g b = (point _)) -> fiber f b)
...
lemma is_short_exact_equiv {A B A' B' : Type} {C C' : pType}
...
Proof.
  ...
  refine ap eC^-1 _ @ point_eq eC^-1ᵉ*, exact is_short_exact.im_in_ker H (eA a) },
  { intro b p, note q . eq_of_inv_eq ((homotopy_top_of_hsquare' h₂ b)^-1 @ p) @ point_eq eC,
  induction is_short_exact.ker_in_im H (eB b) q with a' r,
  apply image.mk (eA^-1 a'),
  exact inj eB ((homotopy_top_of_hsquare h₁^-1ʰᵗʸᵛ a')^-1 @ r) },
...
lemma is_exact_of_is_short_exact {A B : Type} {C : pType}
...
Definition is_contr_right_of_is_short_exact {A B : Type} {C : pType} {f : A -> B} {g : B -> C}
  (H : is_short_exact f g) (HB : is_contr B) (HC : is_set C) : is_contr C.
is_contr_of_is_surjective g (is_short_exact.is_surj H) HB HC

Definition is_contr_left_of_is_short_exact {A B : Type} {C : pType} {f : A -> B} {g : B -> C}
