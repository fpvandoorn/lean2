...
Definition fserre :
  (fun p q => uoH^p[K agℤ 2, H^q[circle₊]]) ⟹ᵍ (fun n => H^n[unit₊]).
  proof
  convergent_exact_couple_g_isomorphism
  (serre_convergence_map_of_is_conn (point _) f (EM_spectrum agℤ) 0
...
exit   section
  ...
  local notation `E∞`  . convergence_Definition.Einf (converges_to.HH fserre)
  local notation `E∞d` . convergence_Definition.Einfdiag (converges_to.HH fserre)
...
Definition fEinfd (n : ℤ) (m : ℕ) (p : n ≠ 0) : is_contr (E∞d (n, 0) m).
  have p' : -n ≠ 0, from fun H => p (eq_zero_of_neg_eq_zero H),
...
Definition is_contr_fD (n s : ℤ) (p : s > 0) : is_contr (E (n, s)).
  ...
  @(is_contr_equiv_closed (left_module.equiv_of_isomorphism (converges_to.e fserre (n, s))^-1ˡᵐ))
  this
...
Definition is_contr_fD3 (n s : ℤ) (p : s ≤ - 2) : is_contr (E (n, s)).
  ...
  @(is_contr_equiv_closed (group.equiv_of_isomorphism (unreduced_ordinary_cohomology_isomorphism_right _ (uH_circle_of_ge _ this)^-1ᵍ _) @e
  left_module.equiv_of_isomorphism (converges_to.e fserre (n, s))^-1ˡᵐ))
...
Proof.
  refine (Einf_isomorphism fserre 0 _ _)^-1ˡᵐ @lm fEinf0,
...
Proof.
  ...
  exact ap (fun x => x - - (1 : ℤ)) !neg_add @ !add_sub_cancel
Defined.

Definition uH0 : uH^0[K agℤ 2] <~>g gℤ.
  (Ex0 0)^-1ᵍ @g group_isomorphism_of_lm_isomorphism_int fE00
