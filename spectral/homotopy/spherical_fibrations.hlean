...
Definition pointed_BG [instance] (n : ℕ) [is_succ n] : pointed (BG n).
  pointed.mk ⟨ S (pred n) , tr erfl ⟩

Definition pBG (n : ℕ) [is_succ n] : pType . pointed.mk' (BG n)

Definition G (n : ℕ) [is_succ n] : Type₁.
  (point _) = (point _) :> BG n
...
Definition pointed_BF [instance] (n : ℕ) : pointed (BF n).
  pointed.mk ⟨ S n , tr pequiv.rfl ⟩

Definition pBF (n : ℕ) : pType . pointed.mk' (BF n)
...
Definition BF_of_BG_mul (n m : ℕ) [is_succ n] [is_succ m] (X : BG n) (Y : BG m)
  ...
  sorry
    ...
    variables {X : Type} {n : ℕ} (α : X -> BF n)
        ...
        protectedDefinition sec (x : X) : carrier (sigma.pr1 (α x)).
    Point _

    open pushout sigma

Definition thom_space : Type.
    pushout (fun x : X => ⟨x , thom.sec α x⟩) (const X unit.star)
Defined. thom
...
(*
  ...
  - forall ₁(BF n)=forall ₁(BG n)=ℤ/2ℤ
  - double covers BSG and BSF
  - O : BF n -> BG 1 = Σ(A : Type), ∥ A = bool ∥
  - BSG n = sigma O
  - forall ₁(BSG n)=forall ₁(BSF n)=O
  - BSO(n),
...
Definition tau : S 2 -> BG 2.
    begin
      intro v, induction v with x, do 2 exact (point _),
      exact sorry
    end

Defined. two_sphere

Defined. spherical_fibrations
